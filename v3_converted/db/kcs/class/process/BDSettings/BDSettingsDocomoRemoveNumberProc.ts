...
//2010/10/01 石崎 作成
//
//@uses BDSettingsDocomoRemoveNumberView
//@package BDSettings
//@subpackage Process
...
//@return void
//
class BDSettingsDocomoRemoveNumberProc extends BDSettingsBaseProc {
	constructor(H_param: {} | any[] = Array()) {
		super(H_param);
	}

	getView() {
		if (!(undefined !== this.O_view) || !(this.O_view instanceof BDSettingsDocomoRemoveNumberView)) {
			this.O_view = new BDSettingsDocomoRemoveNumberView();
		}
	...
	doExecute(H_param: {} | any[] = Array()) {
		...
		if (false === this.getView().getSubmitFlag()) {
			this.getView().makeFormElements(O_Clamp.getClampString(O_Clamp.getCarId()));
		} else {
