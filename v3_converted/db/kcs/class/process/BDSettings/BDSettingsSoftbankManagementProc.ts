...
require("view/BDSettings/BDSettingsSoftbankManagementView.php");
...
//@return void
//
class BDSettingsSoftbankManagementProc extends BDSettingsBaseProc {
	constructor(H_param: {} | any[] = Array()) {
		super(H_param);
	}

	getView() {
		if (!(undefined !== this.O_view) || !(this.O_view instanceof BDSettingsSoftbankManagementView)) {
			this.O_view = new BDSettingsSoftbankManagementView();
		}
	...
	doExecute(H_param: {} | any[] = Array()) //請求ダウンロード設定を取得
	{
		...
		if (!this.getModel().checkCarrier(this.getView().gSess().pactid, this.getView().getSet().car_softbank)) {
			this.errorOut(6, "carid:" + this.getView().getSet().car_softbank + "\u304C\u3001pactid:" + this.getView().gSess().pactid + "\u306B\u7D50\u3073\u3064\u3044\u3066\u304A\u308A\u307E\u305B\u3093", 0);
			throw die();
		}
		...
		this.getView().setAssign("clamps", this.getModel().getClamps(this.getView().gSess().pactid, this.getView().getSet().car_softbank));
		this.getView().setAssign("tels", this.getModel().getParentTels(this.getView().gSess().pactid, this.getView().getSet().car_softbank));
