...
require("view/BDSettings/BDSettingsDocomoManagementView.php");
...
//@return void
//
class BDSettingsDocomoManagementProc extends BDSettingsBaseProc {
	constructor(H_param: {} | any[] = Array()) {
		super(H_param);
	}

	getView() {
		if (!(undefined !== this.O_view) || !(this.O_view instanceof BDSettingsDocomoManagementView)) {
			this.O_view = new BDSettingsDocomoManagementView();
		}
	...
	doExecute(H_param: {} | any[] = Array()) //請求ダウンロード設定を取得
	{
		...
		this.getView().setAssign("clamps", this.getModel().getClamps(this.getView().gSess().pactid, this.getView().getSet().car_docomo));
		this.getView().setAssign("tels", this.getModel().getParentTels(this.getView().gSess().pactid, this.getView().getSet().car_docomo));
