...
//@return void
//
class BDSettingsAuRemoveClampProc extends BDSettingsDocomoRemoveClampProc {
	constructor(H_param: {} | any[] = Array()) {
		super(H_param);
	}

	getView() {
		if (!(undefined !== this.O_view) || !(this.O_view instanceof BDSettingsAuRemoveClampView)) {
			this.O_view = new BDSettingsAuRemoveClampView();
		}
	...
	doExecute(H_param: {} | any[] = Array()) {
		...
		if (false === this.getView().getSubmitFlag()) {
			...
			if (true === O_Clamp.removeClamp(this.getView().gSess().pactid, O_Clamp.getCarId(), H_Get.detailno, H_Post)) {
				this.getView().setAssign("result", H_str.code + "\u3092\u524A\u9664\u3057\u307E\u3057\u305F");
			}
		}

		this.getView().setNaviHTML(this.setNaviArray(H_str.code));
