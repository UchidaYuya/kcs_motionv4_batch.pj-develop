...
//@uses SuoImportSoftTeleView
//@uses SuoImportSoftTeleModel
//
//
...
require("view/script/SuoImportSoftTeleView.php");

require("model/script/SuoImportSoftTeleModel.php");
...
//@return void
//
class SuoImportSoftTeleProc extends ProcessBaseBatch {
	...
	doExecute(H_param: {} | any[] = Array()) //メモリが足りないので・・
	//SUOメモリ足りないので上げる
	//処理開始
	//固有ログディレクトリの作成取得
	//スクリプトの二重起動防止ロック
	...
	//終了
	{
		...
		var dataDir = this.getSetting().KCS_DIR + this.getSetting().KCS_DATA + "/" + this.BillDate + this.getSetting().SBTELECOM_DIR + "/";
		...
		//正常処理が完了した pactid のみリストに追加
		{
			if (undefined !== H_pactid[A_pactid[pactCounter]] == false) //次のpactidへスキップ
				...
				//ログ出力
				{
					...
					var H_dummyData = O_BillModel.getDummy(A_pactid[pactCounter], this.getSetting().CARID, "", tableNo);
					...
					//データファイルチェックでエラーがあった場合（項目数）
					{
						...
						if (rtnCode == false) //データファイルチェックでエラーがなかった場合
							...
							{
								A_allSumData = array_merge(A_allSumData, A_sumData);
								A_allBillData = array_merge(A_allBillData, A_billData);
								A_allTuwaData = array_merge(A_allTuwaData, A_tuwaData);
							}
					}
			...
			//tel_X_tb へ登録する必要有りの場合
			{
				...
				{
					if (undefined !== H_utiwake[this.getSetting().CARID][H_editAllBillData[telno][detailno].code] == false) {
					...
					A_outPutTelDetails.push({
						...
						codename: H_utiwake[this.getSetting().CARID][H_editAllBillData[telno][detailno].code].name,
					...
					if (H_utiwake[this.getSetting().CARID][H_editAllBillData[telno][detailno].code].codetype == this.getSetting().SAIKEI_NORMAL) //合計金額を算出
						{
			...
			if (+(sumCharge / 100 - sumChargeInt != 0)) //tel_details_X_tb 出力用配列へ格納
				//tel_X_tb に電話番号がない場合
				{
					...
					A_outPutTelDetails.push({
						...
						code: this.getSetting().UTIWAKE_TYOUSEI_CODE,
						codename: H_utiwake[this.getSetting().CARID][this.getSetting().UTIWAKE_TYOUSEI_CODE].name,
						charge: +(sumCharge / 100 - sumChargeInt),
						taxkubun: this.getSetting().A_TAX_KUBUN[H_utiwake[this.getSetting().CARID][this.getSetting().UTIWAKE_TYOUSEI_CODE].taxtype],
						detailno: detailno,
						recdate: now,
						carid: this.getSetting().CARID,
						tdcomment: undefined,
					...
					if (-1 !== H_telnoX[this.getSetting().CARID].indexOf(H_dummyData[0].telno) == false) //tel_X_tb 出力用配列へ格納
						{
							H_data = {
								pactid: A_pactid[pactCounter],
		...
		if ("Y" == this.BackUpFlg) //請求明細バックアップファイル名
			...
			//通話明細をバックアップ
			{
				...
				expFile = dataDir + commhistory_tb + date("YmdHis") + ".exp";
		...
		if (undefined !== A_outPutCommhistory == true && 0 != A_outPutCommhistory.length) //出力件数取得
			//一定行数以上は分割して複数回で取込む
			{
				...
				{
					var A_doCommhistory = A_outPutCommhistory.slice(doCnt * this.getSetting().NUM_FETCH, this.getSetting().NUM_FETCH);
