...
//@uses SuoImportKaunetView
//@uses SuoImportKaunetModel
//
//
...
require("view/script/SuoImportKaunetView.php");

require("model/script/SuoImportKaunetModel.php");
...
//@return void
//
class SuoImportKaunetProc extends ProcessBaseBatch {
	...
	//終了
	{
		...
		var dataDir = this.getSetting().KCS_DIR + this.getSetting().KCS_DATA + "/" + this.BillDate + this.getSetting().KOUBAI_KAUNET_DIR + "/";
					...
					//売上ＮＯ毎に消費税を出力
					{
						...
						//売上行ＮＯ一覧を１件ずつ処理 END
						{
							...
							//purchase_X_tb へ登録する必要有りの場合、一番最初の行の会社情報を使用する
							{
								...
								if (this.getSetting().TAX_SOTOZEI == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].taxkubun) //消費税を加算
									{
										sumCharge = sumCharge + charge;
									}

								if (this.getSetting().AKADEN_ORDER == akakuro) {
						...
						if (0 != +(sumCharge * this.getSetting().excise_tax)) //purchase_details_X_tb 出力用配列へ格納
							{
								...
								A_outPutPurchaseDetails.push({
									...
									code: code,
									codename: H_utiwake[this.getSetting().CARID][code].name,
									charge: +(sumCharge * this.getSetting().excise_tax),
									slipno: uriageno,
									itemcode: undefined,
									itemname: "\u6D88\u8CBB\u7A0E",
									itemsum: undefined,
