...
require("view/script/ImportSoftBankView.php");

require("model/script/ImportSoftBankModel.php");
...
//@return void
//
class ImportSoftBankBillProc extends ProcessBaseBatch {
	...
	//tel_details_tb へ登録
	{
		...
		for (var H_row of Object.values(A_allFileData)) //tel_X_tb に電話番号がない場合
		{
			...
			if (!!H_row.telno && -1 !== H_telnoX[this.getSetting().SOFTBANK_CARID].indexOf(H_row.telno) == false) //tel_X_tb へ登録する必要有り
				//電話番号リストに追加
				//現在テーブルにも追加する場合（過去テーブルに無いときだけ追加）
				{
					this.O_Model.setOutPut(this.manageX_tb, A_outPut[this.manageX_tb], H_row);
					H_telnoX[this.getSetting().SOFTBANK_CARID].push(H_row.telno);

					if ("N" == this.TargetTable) //tel_tb に電話番号がない場合
						{
							H_row.postid = rootPostid;

							if (-1 !== H_telno[this.getSetting().SOFTBANK_CARID].indexOf(H_row.telno) == false) //tel_tb へ登録する必要有り
								//電話番号リストに追加
								{
									this.O_Model.setOutPut(this.manage_tb, A_outPut[this.manage_tb], H_row);
									H_telno[this.getSetting().SOFTBANK_CARID].push(H_row.telno);
								}
						}
				}
		}
	...
	//basenameから拡張子を抜く
	{
		...
		var A_clamps = O_model.getClamps(pactid, this.getSetting().SOFTBANK_CARID);
		var flag = false;
	...
	{
		var H_utiwake = this.getBillModel().getUtiwake([this.getSetting().SOFTBANK_CARID]);
		this.H_utiwake = H_utiwake[this.getSetting().SOFTBANK_CARID];
	}
