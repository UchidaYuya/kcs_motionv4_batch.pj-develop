//
//アルファパーチェス 購買データ取込処理 （Process）
//
//更新履歴：<br>
//2008/06/05 前田 聡 作成
//
//SuoImportAlphapurchaseProc
//
//@package SUO
//@subpackage Process
...
//@uses SuoImportAlphapurchaseView
//@uses SuoImportAlphapurchaseModel
//
//
...
require("view/script/SuoImportAlphapurchaseView.php");

require("model/script/SuoImportAlphapurchaseModel.php");
...
//@return void
//
class SuoImportAlphapurchaseProc extends ProcessBaseBatch {
	...
	//終了
	{
		...
		var dataDir = this.getSetting().KCS_DIR + this.getSetting().KCS_DATA + "/" + this.BillDate + this.getSetting().KOUBAI_ALPHAPURCHASE_DIR + "/";
					...
					//送料があった場合は送料を出力
					{
						...
						//売上行ＮＯ一覧を１件ずつ処理 END
						{
							...
							//送料がある場合
							{
								...
								if (this.getSetting().ECO_FLG == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green1 || this.getSetting().ECO_FLG == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green2 || this.getSetting().ECO_FLG == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green3) //通常商品の場合
									{
						...
						code = this.getSetting().UTIWAKE_ORN_CODE;
						A_outPutPurchaseDetails.push({
							pactid: A_pactid[pactCounter],
							purchid: koubai_id,
							purchcoid: this.getSetting().CARID,
						...
						outPurchdetailsCnt++;
						code = this.getSetting().UTIWAKE_SHN_CODE;
						A_outPutPurchaseDetails.push({
							pactid: A_pactid[pactCounter],
							purchid: koubai_id,
							purchcoid: this.getSetting().CARID,
						...
						if ("" != delcharge) //purchase_details_X_tb 出力用配列へ格納
							{
								code = this.getSetting().UTIWAKE_DEL_CODE;
								A_outPutPurchaseDetails.push({
									pactid: A_pactid[pactCounter],
									purchid: koubai_id,
									purchcoid: this.getSetting().CARID,
									...
									slipno: uriageno,
									itemcode: undefined,
									itemname: H_utiwake[this.getSetting().CARID][code].name,
									itemsum: undefined,
									buydate: uriagebi,
									comment: undefined,
						...
						if (0 != sum) //purchase_details_X_tb 出力用配列へ格納
							{
								code = this.getSetting().UTIWAKE_SUM_CODE;
								A_outPutPurchaseDetails.push({
									pactid: A_pactid[pactCounter],
									purchid: koubai_id,
									purchcoid: this.getSetting().CARID,
						...
						if (0 != sum || 0 != sumTax) //purchase_details_X_tb 出力用配列へ格納
							{
								code = this.getSetting().UTIWAKE_SUX_CODE;
								A_outPutPurchaseDetails.push({
									pactid: A_pactid[pactCounter],
									purchid: koubai_id,
									purchcoid: this.getSetting().CARID,
