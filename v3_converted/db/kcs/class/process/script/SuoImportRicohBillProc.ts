//
//リコーコピー機請求データ取込処理 （Process）
//
//更新履歴：<br>
//2008/06/27 前田 聡 作成
//
//SuoImportRicohBillProc
//
//@package SUO
//@subpackage Process
...
//@uses SuoImportRicohBillView
//@uses SuoImportRicohBillModel
//
//
...
require("model/CopyModel.php");

require("model/FuncModel.php");

require("process/ProcessBaseBatch.php");

require("view/script/SuoImportRicohBillView.php");

require("model/script/SuoImportRicohBillModel.php");
...
//@return void
//
class SuoImportRicohBillProc extends ProcessBaseBatch {
	...
	//終了
	{
		...
		var dataDir = this.getSetting().KCS_DIR + this.getSetting().KCS_DATA + "/" + this.BillDate + this.getSetting().COPY_RICOH_BILL_DIR + "/";
		...
		var outCopyCnt = outCopyXCnt = outCopydetailsCnt = 0;
		...
		//正常処理が完了した pactid のみリストに追加
		{
			...
			if (-1 !== Object.keys(H_pactFunc).indexOf("fnc_asp") == true) //ＡＳＰ料金を取得
				{
					aspFlg = true;
					var aspCharge = O_BillModel.getCopyAspCharge(A_pactid[pactCounter], this.getSetting().COPYCOID);
			...
			if ("N" == this.TargetTable) //現在テーブルより削除されていないコピー機ＩＤマスターを取得する array(copyid)
				...
				//削除フラグをfalseにする必要のあるコピー機ＩＤリスト
				{
					var A_copyid = O_CopyModel.getCopyid(A_pactid[pactCounter], this.getSetting().COPYCOID, "false");
					var A_copyidDel = O_CopyModel.getCopyid(A_pactid[pactCounter], this.getSetting().COPYCOID, "true");
			...
			//copy_X_tb へ登録する必要有りの場合、一番最初の行の会社情報を使用する
			{
				...
				var code = this.getSetting().UTIWAKE_CODE;
				...
				A_outPutCopyDetails.push({
					...
					code: code,
					codename: H_utiwake[this.getSetting().COPYCOID][code].name,
				...
				if (true == aspFlg) //合計用に１つ進める
					//ASP利用料を出力
					{
						...
						A_outPutCopyDetails.push({
							...
							codename: H_utiwake[this.getSetting().COPYCOID][this.getSetting().UTIWAKE_ASP_CODE].name,
							charge: aspCharge,
							taxkubun: "\u500B\u5225",
							printcount: undefined,
							detailno: detailno,
						...
						if (0 != asxCharge) //ASP利用料消費税を出力
							{
								...
								A_outPutCopyDetails.push({
									...
									codename: H_utiwake[this.getSetting().COPYCOID][this.getSetting().UTIWAKE_ASX_CODE].name,
									charge: asxCharge,
									taxkubun: undefined,
									printcount: undefined,
									detailno: detailno,
			...
			outCopyCnt = outCopyXCnt = outCopydetailsCnt = 0;
		}
		...
		if ("Y" == this.BackUpFlg) {
			...
			if (false == this.expData(copydetails_tb, expFile, this.getSetting().NUM_FETCH)) {
				throw die(-1);
			}
		}
		...
		for (var pactDoneCounter = 0; pactDoneCounter < pactDoneCnt; pactDoneCounter++) //コピー機ＩＤの削除フラグをfalseにする必要がある場合
		{
			if (A_undoCopyidX.length != 0) {
				O_CopyModel.chgDelFlg(A_pactDone[pactDoneCounter], this.getSetting().COPYCOID, A_undoCopyidX, "false", tableNo);
			}

			if (undefined !== A_undoCopyid == true && 0 != A_undoCopyid.length) {
				O_CopyModel.chgDelFlg(A_pactDone[pactDoneCounter], this.getSetting().COPYCOID, A_undoCopyid, "false");
			}
		}
