...
require("view/Admin/Product/AdminProductEditStockView.php");
...
//@return void
//
class AdminProductEditStockProc extends ProcessBaseHtml {
	constructor(H_param: {} | any[] = Array()) {
		super(H_param);
	}

	get_View() {
		return new AdminProductEditStockView({
	...
	//登録する前
	{
		...
		var property = O_model.getBranchProperty(O_view.gSess().admin_groupid, O_view.getSProductIdValue(), O_view.getBranchID());
		var err = false;

		if (0 != O_model.checkProductProperty(O_view.getSProductIdValue(), O_view.getBranchProperty()).length && property != O_view.getBranchProperty()) {
			err = true;
		}

		O_view.makeFormElements(property, err);
		...
		if (false == O_view.getSubmitFlag()) {
			...
			O_model.updateProductBranchProperty(O_view.getSProductIdValue(), O_view.getBranchID(), O_view.getBranchProperty());
