...
require("view/Bill/ICCard/ICCardPrintOutKGView.php");

require("model/Bill/ICCard/ICCardPrintOutKGModel.php");
...
//@return void
//
class ICCardPrintOutKGProc extends ProcessBaseHtml {
	static PUB = "/Bill";
	...
	get_View() {
		return new ICCardPrintOutKGView();
	}

	get_Model(H_g_sess) {
		return new ICCardPrintOutKGModel(this.get_DB(), H_g_sess);
	}
	...
	//Smartyによる表示
	{
		...
		var docmonth = _SESSION["/Bill/ICCard/ICCardPrintOut.php,docmonth"];
		var H_data = O_model.getList(pactid, coid, userid, tblno);
		var A_userinfo = O_model.getUserInfo(userid);
		O_view.outputPDF(H_data, A_userinfo, printdate, docmonth);
	}
