...
require("view/Bill/AddBill/AddBillUnsettledView.php");

require("model/Bill/AddBill/AddBillUnsettledModel.php");
...
//@return void
//
class AddBillUnsettledProc extends ProcessBaseHtml {
	static PUB = "/Bill";
	...
	get_View() {
		return new AddBillUnsettledView();
	}

	get_Model(H_g_sess) {
		return new AddBillUnsettledModel(this.get_DB(), H_g_sess);
	}
	...
	//フォームにエラーがないなら
	{
		...
		O_model.initialize(H_sess[AddBillUnsettledProc.PUB].cym);
		var H_list = O_model.getList(H_g_sess.pactid, H_sess.SELF.cfmid, H_sess.SELF.sort);
		var validate = O_view.makeForm();

		if (validate) {
			if (H_sess.SELF.post.submit == O_view.UnsettledName) //CSRF
				//sql文を作成
				//DB更新成功
				{
