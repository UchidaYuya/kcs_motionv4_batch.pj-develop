...
require("view/Bill/AddBill/AddBillDownloadView.php");
...
//@return void
//
class AddBillDownloadProc extends ProcessBaseHtml {
	static PUB = "/Bill";
	...
	get_View() {
		return new AddBillDownloadView();
	}
	...
	//CSV出力
	{
		...
		O_model.initialize(H_sess[AddBillDownloadProc.PUB].cym);
		...
		if (H_sess.SELF.input_flg || H_sess[AddBillDownloadProc.PUB].cym == "current") //受注内容入力について
			{
				userid = -1;
				...
				var H_list = O_model.getList(H_g_sess.pactid, 0, 0, H_sess.SELF.search, H_sess.SELF.sort, userid);
			} else //表示するデータ取得
			{
				H_list = O_model.getBillList(H_g_sess.pactid, H_sess[AddBillDownloadProc.PUB].cym, H_sess[AddBillDownloadProc.PUB].current_postid, H_sess.SELF.coid, H_sess.SELF.sort, H_sess.SELF.search);
			}
