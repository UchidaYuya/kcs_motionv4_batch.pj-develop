...
//@return void
//
class AddBillUnsettledModel extends ModelBase {
	...
	initialize(cym) //テーブルNoの取得
	{
		if (cym == "current") {
			this.m_tb.addbill_tb = "addbill_tb";
		} else {
			var O_table = new MtTableUtil();
			var tbno = O_table.getTableNo(cym);
			this.m_tb.addbill_tb = "addbill_" + tbno + "_tb";
		}
	}
	...
	//対象を一件ずつ処理するぽよ
	{
		...
		for (var value of Object.values(cfmid)) //削除フラグを立てる
		{
			var id = value.split(":");
			...
			res.push("update " + this.m_tb.addbill_tb + " set confirm_flg=false" + where);
		}
