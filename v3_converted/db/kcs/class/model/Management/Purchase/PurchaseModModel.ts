...
//@return void
//
class PurchaseModModel extends ManagementPurchaseModel {
	...
	//対象が無ければエラー
	{
		this.setTableName(H_sess[PurchaseModModel.PUB].cym);
		var H_row = this.get_db().queryRowHash(this.makePurchSelectOneSQL(H_sess.SELF.get.manageno, H_sess.SELF.get.coid));
		...
		var A_post = this.O_Post.getChildList(this.H_G_Sess.pactid, H_sess[PurchaseModModel.PUB].current_postid, this.H_Tb.tableno);
	...
	makeModManageSQL(H_sess, pre = false) {
		...
		var sql = this.makeUpdatePurchaseSQL(H_sess.SELF, tb);
		return sql;
	}
	...
	makeModLogSQL(H_post: {} | any[]) {
		...
		var A_val = [PurchaseModModel.PURCHMID, this.H_G_Sess.pactid, this.H_G_Sess.postid, this.H_G_Sess.userid, this.H_G_Sess.loginname, H_post.purchid, H_post.purchid, H_post.purchcoid, "\u8CFC\u8CB7ID\u5909\u66F4", "Change purhcaseid", H_post.recogpostid, "", H_post.recogpostname, "", this.H_G_Sess.joker, "\u5909\u66F4", this.NowTime];
	...
	checkInputError(H_sess, O_form, chgflg) //既に登録済みかチェック（キー変更時）
	{
		...
		if (undefined !== H_sess.SELF.post.modsubmit == true && chgflg == true) {
			if (this.checkPurchExist(H_sess.SELF.post.purchid, H_sess.SELF.post.purchcoid) == true) {
		...
		if (undefined !== H_sess.SELF.post.modsubmit == true && undefined !== H_sess.SELF.post.pastflg == true && H_sess.SELF.post.pastflg == 1) //配下の部署一覧
			//存在チェック
			{
				...
				var cnt = this.checkPrePurchAuth(H_sess.SELF.get.manageno, H_sess.SELF.get.coid, A_prepostid);

				if (cnt < 1) {
					O_form.setElementErrorWrapper("pastflg", "\u524D\u6708\u306B\u5B58\u5728\u3057\u306A\u3044\u8CFC\u8CB7ID\u3067\u3059\u3002\u524D\u6708\u5206\u3082\u5909\u66F4\u306E\u30C1\u30A7\u30C3\u30AF\u3092\u5916\u3057\u3066\u304F\u3060\u3055\u3044");
				}
			}
	}
	...
	makeDelPreManageSQL(H_post, pre = false) {
		...
		var sql = this.makeDelPurchSQL(H_post.pre_purchid, H_post.pre_purchcoid, tb);
		return sql;
	}
