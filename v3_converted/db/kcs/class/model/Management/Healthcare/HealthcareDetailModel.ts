...
//@return void
//
class HealthcareDetailModel extends ManagementHealthcareModel {
	...
	//配下の部署一覧取得
	{
		...
		var healthid = A_id[0];
		var healthcoid = A_id[1];
		this.setTableName(H_sess[HealthcareDetailModel.PUB].cym);
		var A_post = this.O_Post.getChildList(this.H_G_Sess.pactid, this.H_G_Sess.postid, this.H_Tb.tableno);
		var H_data = this.get_db().queryRowHash(this.makeHealthSelectOneSQL(healthid, healthcoid));
		var H_co = this.getHealthcareCoData();

		if (H_data.length > 1) //tablenoもviewに渡す
			//配下の運送IDで無ければ消す
			{
				H_data.tableno = this.H_Tb.tableno;
				H_data.healthconame = H_co[H_data.healthcoid];
	...
	getViewProperty() {
		var H_prop = this.getManagementProperty(HealthcareDetailModel.HEALTHMID);
		return H_prop;
	}
