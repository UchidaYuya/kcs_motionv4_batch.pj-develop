...
//@return void
//
class ShopOrderListViewModel extends OrderListMenuModel {
	...
	constructor(O_db0, H_g_sess) {
		...
		this.O_ordshpmodel = new ShopOrderModelBase(O_db0, H_g_sess, ShopOrderListViewModel.SITE_SHOP);
	}
	...
	selectSql() {
		return ["ord.orderid", "ord.pacttype", "ord.status", "ord.ordertype", "ord.transfer", "ord.carid", "ord.buyselid", "ord.postname", "ord.ansdate", "ord.telcnt", "ord.receipt", "ptn.ptnname", "ord.misctype", "ord.completedate", "ord.attachedfile", "ord.attachedshop", "ord.addr1", "ord.division"].join(",");
	}
	...
	groupBySql() {
		var A_col = ["ord.orderid", "ord.status", "ord.ansdate", "ord.pacttype", "ord.ordertype", "ord.transfer", "ord.carid", "ord.buyselid", "ord.postname", "ord.telcnt", "ord.receipt", "ptn.ptnname", "ord.misctype", "ord.completedate", "ord.attachedfile", "ord.attachedshop", "ord.addr1", "ord.division"];
		return " GROUP BY " + A_col.join(",");
	}
	...
	//注文日付のソート付加
	{
		...
		if (A_key[0] != 7) sql += ",ord.ansdate desc";
		if (A_key[0] != 11) sql += ",ord.orderid";
		sql += ",sub.ordersubid";
		return sql;
	}

	getTransfer(H_g_sess, A_shopid) {
		var temp = new ShopOrderModelBase(O_db0, H_g_sess, ShopOrderListViewModel.SITE_SHOP);
	...
	getTransferShopId(shopid, H_shoplist, type, orderids) {
		...
		var sql = "SELECT " + material + " " + "FROM " + "mt_transfer_tb " + "WHERE " + column + " IN (" + shopid.join(", ") + ") " + " AND orderid IN (" + orderids.join(", ") + ") " + "GROUP BY " + material;
		...
		sql = "SELECT orderid, " + column + " AS shopid " + "FROM " + "mt_transfer_tb " + "WHERE " + material + " IN (" + A_trans.join(", ") + ") " + " AND orderid IN (" + orderids.join(", ") + ") " + "ORDER BY detail_sort, transfer_level";
	...
	getChargeCount2(shopid, orderids) {
		var sql = "SELECT o.orderid, o.ordertype, t.maccnt, t.acccnt " + "FROM " + "mt_transfer_charge_shop_tb t " + "INNER JOIN mt_order_tb o ON t.orderid=o.orderid " + "WHERE " + "t.shopid IN (" + shopid.join(", ") + ") AND t.orderid IN (" + orderids.join(", ") + ")";
		var H_temp = this.get_DB().queryHash(sql);
	...
	getChargeCount3(shopid, orderids) //ショップ、注文がない場合は何もしない
	{
		if (!shopid || !orderids) {
			return Array();
		}

		var sql = "SELECT" + " ord.orderid" + ",sum( CASE WHEN ord.ordertype = '" + this.O_order.type_acc + "' THEN  0 ELSE trans.maccnt END ) as maccnt" + ",sum( trans.acccnt ) as acccnt" + " FROM" + " mt_transfer_charge_shop_tb trans " + " INNER JOIN mt_order_tb ord ON" + " ord.orderid=trans.orderid" + " and ord.orderid in (" + orderids.join(", ") + ")" + " WHERE " + " trans.shopid IN (" + shopid.join(", ") + ")" + " GROUP BY" + " ord.orderid";
		return this.get_DB().queryKeyAssoc(sql);
	}
	...
	getList(H_sess, H_box, shopid) //包括販売店で配下の販売店が選択されていたらshopidをしぼる
	{
		if (true == H_box.unify) {
			if (true == (undefined !== H_sess[ShopOrderListViewModel.PUB].sctcode) && undefined != H_sess[ShopOrderListViewModel.PUB].sctcode && "0" != H_sess[ShopOrderListViewModel.PUB].sctcode) {
				shopid = [H_sess[ShopOrderListViewModel.PUB].sctcode];
			}
		}

		var orderby = this.makeOrderBySQL(H_sess[ShopOrderListViewModel.PUB]);
		var sql = "SELECT " + this.makeOrderSelectSQL() + this.fromSql(shopid) + this.searchwhere + this.makeGroupBySQL();
		this.count = this.get_DB().queryOne("SELECT count(*) FROM ( " + sql + ") AS temp");
		H_sql.search = sql;
		this.H_sql = H_sql;
		orderby = this.makeOrderBySQL(H_sess[ShopOrderListViewModel.PUB]);
		sql += orderby;
		var offset = (H_sess[ShopOrderListViewModel.PUB].offset - 1) * H_sess[ShopOrderListViewModel.PUB].limit;
		var limit = H_sess[ShopOrderListViewModel.PUB].limit;
		sql += " LIMIT " + limit + " OFFSET " + offset;
		return this.get_DB().queryHash(sql);
	}

	getList2(H_sess, H_box, shopid) //包括販売店で配下の販売店が選択されていたらshopidをしぼる
	//offsetとlimit
	//getListから必要そうなもの持ってきた。いずれこのコードは消し去りたい
	//振替先の検索がある場合
	//対象のorderidをここで絞る
	//sql_listのorderidにJOIN、OrderBy、LIMIT、OFFSETつける
	{
		if (true == H_box.unify) {
			if (true == (undefined !== H_sess[ShopOrderListViewModel.PUB].sctcode) && undefined != H_sess[ShopOrderListViewModel.PUB].sctcode && "0" != H_sess[ShopOrderListViewModel.PUB].sctcode) {
				shopid = [H_sess[ShopOrderListViewModel.PUB].sctcode];
		...
		if (!!H_sess[ShopOrderListViewModel.PUB].trans) {
			var parent = "SELECT parentshop FROM support_shop_tb WHERE childshop=" + this.get_DB().dbQuote(H_sess[ShopOrderListViewModel.PUB].trans, "int", true);
			var A_parent = this.get_DB().queryCol(parent);
			A_parent.push(+H_sess[ShopOrderListViewModel.PUB].trans);
			var sql_trans = " LEFT JOIN mt_transfer_tb trans ON trans.orderid = ord.orderid AND trans.toshopid IN (" + A_parent.join(", ") + ") AND trans.fromshopid IN (" + sql_shop + ")";
		}

		if (!!H_sess[ShopOrderListViewModel.PUB].post.deliverydate_type || !!H_sess[ShopOrderListViewModel.PUB].post.send_deliverydate_flg) {
			var sql_deliverydate_mail = " INNER JOIN fnc_relation_tb frel ON frel.userid = 0 AND frel.pactid = ord.pactid AND frel.fncid=233";
		} else {
			sql_deliverydate_mail = "";
		}

		var sql_list = "select" + " ord.orderid" + ",min(CASE WHEN ord.ordertype = 'A' THEN 1 ELSE sub.ordersubid END) as sub_subid" + ",min(det.ordersubid) as det_subid" + ",min(det.detail_sort) as det_sort" + " from" + " mt_order_tb as ord" + " INNER JOIN mt_transfer_charge_shop_tb trch ON (ord.orderid=trch.orderid AND trch.shopid IN (" + sql_shop + "))" + " INNER JOIN mt_order_sub_tb sub ON ( sub.orderid=ord.orderid)" + " INNER JOIN mt_order_teldetail_tb det ON ( det.orderid=ord.orderid)" + sql_trans + sql_deliverydate_mail + " " + this.searchwhere + " group by ord.orderid";
		sql = "select" + " ord.orderid" + ",ord.pacttype" + ",ord.status" + ",ord.ordertype" + ",ord.transfer" + ",ord.carid" + ",sub.productname" + ",sub.property" + ",ord.buyselid" + ",ord.postname" + ",ord.ansdate" + ",ord.telcnt" + ",ord.receipt" + ",ptn.ptnname" + ",det.contractor" + ",sub.expectdate" + ",sub.deliverydate" + ",sub.fixdate" + ",shmem.name as memname" + ",ord.misctype" + ",sub.machineflg" + ",ord.completedate" + ",ord.attachedfile" + ",ord.attachedshop" + ",ord.addr1" + ",ord.division" + ",ord.retransfer_from_shopid" + ",ord.retransfer_to_shopid" + ",retrans_from.postcode as rtr_f_postcode" + ",retrans_from.name as rtr_f_name" + ",retrans_to.postcode as rtr_t_postcode" + ",retrans_to.name as rtr_t_name" + ",count(*) over() as full_count" + " from" + " (" + sql_list + ") as id_list" + " INNER JOIN mt_order_tb ord ON ord.orderid = id_list.orderid" + " INNER JOIN mt_order_sub_tb sub on sub.orderid=id_list.orderid" + " and sub.ordersubid=id_list.sub_subid" + " INNER JOIN mt_order_pattern_tb ptn ON ord.ordertype=ptn.type AND ord.carid=ptn.carid AND ord.cirid=ptn.cirid" + " INNER JOIN pact_tb pact ON ord.pactid=pact.pactid" + " LEFT JOIN mt_order_teldetail_tb det on det.orderid=id_list.orderid" + " and det.ordersubid = id_list.det_subid" + " and det.detail_sort= id_list.det_sort" + " LEFT JOIN shop_member_tb shmem ON ord.shopmemid=shmem.memid" + " LEFT JOIN shop_tb retrans_from ON retrans_from.shopid = ord.retransfer_from_shopid" + " LEFT JOIN shop_tb retrans_to ON retrans_to.shopid = ord.retransfer_to_shopid" + " " + this.makeOrderBySQL(H_sess[ShopOrderListViewModel.PUB]) + " LIMIT " + limit + " OFFSET " + offset;
		var res = this.get_DB().queryHash(sql);

		if (!res) {
			this.count = 0;
		} else {
			this.count = res[0].full_count;
		}
	...
	makeSearchSql(H_sess, H_box, shopid) //包括販売店で配下の販売店が選択されていたらshopidをしぼる
	{
		...
		this.searchwhere = this.makeSearchWhere(H_sess[ShopOrderListViewModel.PUB], H_box, shopid);
		var orderby = this.makeOrderBySQL(H_sess[ShopOrderListViewModel.PUB]);
	...
	fromSql(shopid) {
		var sql = " FROM " + ShopOrderListViewModel.ORD_TB + " ord " + "INNER JOIN (" + this.subSql(shopid) + ") sub on ord.orderid=sub.orderid " + "LEFT JOIN (" + this.teldetailSql(shopid) + ") det on ord.orderid=det.orderid " + "LEFT JOIN mt_transfer_tb trans ON (ord.orderid=trans.orderid AND (det.detail_sort=trans.detail_sort OR sub.detail_sort=trans.detail_sort)) " + "INNER JOIN mt_transfer_charge_shop_tb trch ON (ord.orderid=trch.orderid AND trch.shopid IN (" + shopid.join(", ") + ")) " + "INNER JOIN mt_order_pattern_tb ptn ON ord.ordertype=ptn.type AND ord.carid=ptn.carid AND ord.cirid=ptn.cirid " + "INNER JOIN pact_tb pact ON ord.pactid=pact.pactid " + "LEFT JOIN shop_member_tb shmem ON ord.shopmemid=shmem.memid ";
		return sql;
	}

	subSql(shopid) {
		var sql = "SELECT orderid, ordersubid, productname, property, expectdate, deliverydate, fixdate, machineflg, detail_sort FROM ( SELECT s.orderid, min(ordersubid) as ordersubid FROM " + ShopOrderListViewModel.ORD_SUB_TB + " AS s INNER JOIN " + ShopOrderListViewModel.ORD_TB + " AS o ON o.orderid = s.orderid AND ( o.shopid IN (" + shopid.join(", ") + ") OR o.orderid IN (select orderid from mt_transfer_tb where toshopid in (" + shopid.join(", ") + ") )) GROUP BY s.orderid ) AS sub INNER JOIN " + ShopOrderListViewModel.ORD_SUB_TB + " USING(orderid, ordersubid)";
		return sql;
	}

	teldetailSql(shopid) {
		var sql = "SELECT orderid, contractor, ordersubid, detail_sort FROM ( SELECT d.orderid, min(ordersubid) as ordersubid, min(detail_sort) as detail_sort FROM " + ShopOrderListViewModel.ORD_DET_TB + " AS d INNER JOIN " + ShopOrderListViewModel.ORD_TB + " AS o ON o.orderid = d.orderid AND ( o.shopid IN (" + shopid.join(", ") + ") OR o.orderid IN (select orderid from mt_transfer_tb where toshopid in (" + shopid.join(", ") + ") )) GROUP BY d.orderid ) as teldetail INNER JOIN " + ShopOrderListViewModel.ORD_DET_TB + " USING(orderid, ordersubid, detail_sort)";
		return sql;
	}

	makeOrderTableSQL(shopid) {
		var sql = " FROM " + ShopOrderListViewModel.ORD_TB + " ord " + "INNER JOIN " + ShopOrderListViewModel.ORD_SUB_TB + " sub on ord.orderid=sub.orderid " + "LEFT JOIN " + ShopOrderListViewModel.ORD_DET_TB + " det on ord.orderid=det.orderid " + "LEFT JOIN mt_transfer_tb trans ON (ord.orderid=trans.orderid AND (det.detail_sort=trans.detail_sort OR sub.detail_sort=trans.detail_sort)) " + "INNER JOIN mt_transfer_charge_shop_tb trch ON (ord.orderid=trch.orderid AND trch.shopid IN (" + shopid.join(", ") + ")) " + "INNER JOIN mt_order_pattern_tb ptn ON ord.ordertype=ptn.type AND ord.carid=ptn.carid AND ord.cirid=ptn.cirid " + "INNER JOIN pact_tb pact ON ord.pactid=pact.pactid " + "LEFT JOIN shop_member_tb shmem ON ord.shopmemid=shmem.memid ";
		return sql;
	}

	makeOrderSubTableSQL(shopid) {
		var sql = " SELECT order";
	}

	makeSearchWhere(H_sess, H_box, A_shopid) //$planesql = $this->sqlConnect($sql, "AND ").$this->makePlaneSearchWhere($A_shopid, "search");
	//数値IN
	//文字列(そのままwhere句に入れるもの)
	//ステータス
	//like検索
	//今日やるボタンの場合 要望により検索対象を処理日から納品日(nullと未定は含まない）に変更
	//何もチェックされてない項目は-1で検索
	{
		var planesql = this.makePlaneSearchWhere(A_shopid, "search");
		var A_integer_in = ["orderid", "pactid"];
		var A_string = ["contractor", "orderid", "telno"];
		var A_checkbox = ["status", "ordertype", "carid", "buyselid"];
		var A_ord_like = ["note"];
		var A_sub_like = ["productname"];
		...
		if (undefined != H_sess.post) //納品日検索。
			//納品日検索した場合、納品日送信権限が会社にあるかチェックすること
			//納品日メール送信したかどうか
			{
				{
					...
					for (var key in _tmp_0) {
						...
						} else if (key == "misctype") //何か選択されてる時だけ、その他検索つける
							{
								if (Array.isArray(val)) {
									sql += " AND ( (ord.ordertype != 'M') OR (ord.ordertype = 'M' AND " + this.makeSearchSqlCheckBox(val, key, H_box[key], A_shopid) + ") ) ";
								}
						...
						} else if (true == (-1 !== A_integer_in.indexOf(key))) //カンマで区切って配列にする
							{
								var temp = val.split(",");
								var a_in = Array();

								for (var n of Object.values(temp)) {
									if (is_numeric(n)) {
										a_in.push(this.get_DB().dbQuote(n, "integer"));
									}
								}

								if (!!a_in) {
									sql += " AND ( ord." + key + " IN (" + a_in.join(",") + ") ) ";
								}
							} else if (true == (-1 !== A_string.indexOf(key))) {
							sql += this.makeSearchSqlString(key, val, sql);
						} else if (-1 !== A_ord_like.indexOf(key)) {
							if (!!val) {
								sql += " AND ord." + key + " ilike " + this.get_DB().dbQuote("%" + val + "%", "text", true);
							}
						} else if (-1 !== A_sub_like.indexOf(key)) {
							if (!!val) {
								sql += " AND sub." + key + " ilike " + this.get_DB().dbQuote("%" + val + "%", "text", true);
							}
						}
					}
				}
				var key = "send_deliverydate_flg";
				var val = H_sess.post[key];

				if (!!H_sess.post[key]) {
					sql += this.sqlConnectEx(sql, this.makeSearchSqlCheckBox(val, key, H_box[key], A_shopid), "AND ", true);
				}

				key = "deliverydate_type";
				val = H_sess.post[key];
		...
		if ("buyselid" == type) //新方式(バリュー)
			{
				...
				if (1 == H_status[2]) {
					sql = "SELECT buyselid FROM buyselect_tb WHERE loanflg=3";
					A_5G = this.get_DB().queryCol(sql);
					flg = true;
				}
				...
				var A_temp = array_merge(A_none, A_new, A_old, A_5G);
			...
			if (!!A_temp && -1 !== A_temp.indexOf(this.O_Set.car_other)) //全キャリア出すバージョン
				//				$sql = "SELECT a.carid ".
	...
	getOrderNumber(H_order) {
		...
		var sql = "SELECT sub.orderid, sum(sub.number) AS accnum FROM " + ShopOrderListViewModel.ORD_SUB_TB + " sub " + "WHERE sub.orderid IN (" + H_key.join(", ") + ") " + "AND sub.machineflg=false " + "GROUP BY sub.orderid";
	...
	offsetListData(H_sess, H_data) {
		...
		for (var key in H_data) {
			...
			if (offset <= idx && H_sess[ShopOrderListViewModel.PUB].limit > max) //$temp = $this->getOrderNumber(array($val["orderid"] => $val));
	...
	getPageLink(H_sess, cnt) {
		return this.O_order.getPageLink(cnt, H_sess[ShopOrderListViewModel.PUB].limit, H_sess[ShopOrderListViewModel.PUB].offset);
	}
	...
	checkFunc(shopid, memid, func) {
		if (!this.A_funcs) {
			var sql = "SELECT " + "rel.fncid" + ",fnc.ininame" + " FROM " + " shop_fnc_relation_tb rel " + " INNER JOIN shop_function_tb fnc ON rel.fncid = fnc.fncid " + " WHERE " + " rel.shopid=" + this.get_DB().dbQuote(shopid, "integer", true) + " AND rel.memid=" + this.get_DB().dbQuote(memid, "integer", true);
			this.A_funcs = this.get_DB().queryKeyAssoc(sql);
		}

		if ("string" === typeof func) {
			if (-1 !== this.A_funcs.indexOf(func)) {
				return true;
			}
		} else if (is_numeric(func)) {
			if (undefined !== this.A_funcs[func]) {
				return true;
			}
		}
	...
	getDLAuth(H_g_sess) //$sql = "SELECT ".
	//					"rel.fncid ".
	//				"FROM ".
	//					"shop_fnc_relation_tb rel ".
	//					"INNER JOIN shop_function_tb fnc ON rel.fncid=fnc.fncid ".
	//				"WHERE ".
	//					"rel.shopid=" .$H_g_sess["shopid"].
	//					" AND rel.memid=" .$H_g_sess["memid"].
	//					" AND fnc.ininame='fnc_shop_download'";
	//		return $this->get_DB()->queryOne($sql);
	{
		return this.checkFunc(H_g_sess.shopid, H_g_sess.memid, "fnc_shop_download");
	}

	setOrderID(H_data, H_sql) //$H_sql["down"]["where"] = $this->searchwhere;
	//$H_sql["down"]["where"] = "where ord.orderid IN (" .implode(", ", $A_orderid). ") ";
	{
		...
		H_sql.down.where = "WHERE ord.orderid IN (SELECT DISTINCT orderid FROM (" + H_sql.search + ") oid)";
	}
