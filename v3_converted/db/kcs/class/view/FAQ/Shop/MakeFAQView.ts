...
class MakeFAQView extends InquiryBaseView {
	...
	addDefaults() {
		var fncid = this.H_assign.formdefaults;

		if (false == is_null(fncid.fnc)) {
			this.O_Qf.setDefaultsWrapper({
				fnc: [fncid.fnc]
			});
		} else {
			fncid = this.H_assign.selectedfncid;

			if (false == is_null(fncid.fncid)) {
				this.O_Qf.setDefaultsWrapper({
					fnc: [fncid.fncid]
				});
			}
	...
	makeFormElements() {
		...
		this.O_Qf.O_Form.setMaxFileSize(5242880);
