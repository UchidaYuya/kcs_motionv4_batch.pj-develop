...
//@return void
//
class EditFAQView extends InquiryBaseView {
	...
	constructor(H_param = undefined) {
		...
		if (0 < _GET.length && true == (undefined !== _GET.id)) {
			this.gSess().setSelf("faqid", _GET.id);
	...
	checkCGIParam() {
		...
		if (0 < _POST.length && true == (undefined !== _POST.submit) && EditFAQView.SELECT_PACTID === _POST.submit) {
			if (true == (undefined !== _POST.submit)) {
				H_post.submit = _POST.submit;
			}
		...
		if (EditFAQView.SELECT_PACTID === H_post.submit) {
			this.Mode = "selectpact";
			return true;
		}

		if (0 < _POST.length && true == (undefined !== _POST.submit) && EditFAQView.SUBMIT_VALUE === _POST.submit) {
			if (true == (undefined !== _POST.submit)) {
				H_post.submit = _POST.submit;
			}
		...
		if (EditFAQView.SUBMIT_VALUE === H_post.submit) {
