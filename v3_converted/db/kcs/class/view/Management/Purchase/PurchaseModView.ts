...
//@return void
//
class PurchaseModView extends ManagementModViewBase {
	constructor() {
		super();
	}
	...
	//現在の変更のとき
	{
		...
		if (date("Ym") == H_sess[PurchaseModView.PUB].cym) {
			A_tmp = {
				name: "pastflg",
	...
	{
		if (H_sess.post.purchid == H_sess.get.manageno && H_sess.post.purchcoid == H_sess.get.coid) {
			H_sess.post.pre_purchid = "";
			H_sess.post.pre_purchcoid = "";
			return false;
		} else {
			H_sess.post.pre_purchid = H_sess.get.manageno;
			H_sess.post.pre_purchcoid = H_sess.get.coid;
			return true;
		}
	}
	...
	viewChgKeyError() //エラー画面表示
	{
		...
		O_err.display("\u8ACB\u6C42\u304C\u3042\u308B\u8CFC\u8CB7ID\u306E\u8CFC\u8CB7ID\u3001\u8CFC\u8CB7\u5148\u306F\u5909\u66F4\u3067\u304D\u307E\u305B\u3093\u3002", 0, _SERVER.PHP_SELF, "\u623B\u308B");
	}
	...
	//完了画面表示
	{
		...
		O_finish.displayFinish("\u8CFC\u8CB7ID\u5909\u66F4", "/Management/Purchase/menu.php", "\u4E00\u89A7\u753B\u9762\u3078");
	}
