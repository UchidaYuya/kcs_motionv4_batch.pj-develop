...
//@return void
//
class PurchaseDownloadView extends ManagementDownloadViewBase {
	constructor() {
		super();
	}
	...
	{
		this.H_View.A_head = ["\u30B7\u30B9\u30C6\u30E0\u90E8\u7F72ID", "\u90E8\u7F72ID", "\u90E8\u7F72\u540D", "\u8CFC\u8CB7ID", "\u8CFC\u8CB7\u5148ID", "\u30ED\u30B0\u30A4\u30F3ID", "\u8CFC\u8CB7\u5148", "\u767B\u9332\u4F1A\u793E\u540D", "\u767B\u9332\u90E8\u7F72\u540D", "\u767B\u9332\u90F5\u4FBF\u756A\u53F7", "\u767B\u9332\u4F4F\u6240\uFF08\u90FD\u9053\u5E9C\u770C\uFF09", "\u767B\u9332\u4F4F\u6240\uFF08\u5E02\u533A\u753A\u6751\u3001\u756A\u5730\uFF09", "\u767B\u9332\u4F4F\u6240\uFF08\u5EFA\u7269\uFF09", "\u767B\u9332\u96FB\u8A71\u756A\u53F7", "\u767B\u9332FAX\u756A\u53F7", "\u767B\u9332\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9", "\u767B\u9332\u65E5", "\u62C5\u5F53\u8005", "\u793E\u54E1\u756A\u53F7", "\u30E1\u30E2"];
	...
	getFileName(H_sess: {} | any[]) {
		var filename = "\u8CFC\u8CB7ID\u60C5\u5831_" + date("YmdHis") + "." + this.H_View.H_dlprop.extension;
		...
		{
			{
				...
				for (var key in _tmp_0) {
					...
					if (key == "registdate" || preg_match("/^date(\\d)/", key) == true) {
			...
			var A_str = [A_data[cnt].postid, A_data[cnt].userpostid, A_data[cnt].postname, A_data[cnt].purchid, A_data[cnt].purchcoid, A_data[cnt].loginid, A_data[cnt].purchconame, A_data[cnt].registcomp, A_data[cnt].registpost, A_data[cnt].registzip, A_data[cnt].registaddr1, A_data[cnt].registaddr2, A_data[cnt].registbuilding, A_data[cnt].registtelno, A_data[cnt].registfaxno, A_data[cnt].registemail, A_data[cnt].registdate, A_data[cnt].username, employeecode, A_data[cnt].memo];
			...
			if (PurchaseDownloadView.DEBUG != "on") {
