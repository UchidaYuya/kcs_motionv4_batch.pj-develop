...
//@return void
//
class HealthcareModView extends ManagementModViewBase {
	constructor() {
		super();
	}
	...
	//現在の変更のとき
	{
		var A_formelement = this.getHealthcareAddModFormElement(O_manage, O_model, H_sess);
		var A_tmp = {
			name: "modsubmit",
			label: this.NextName,
			inputtype: "submit"
		...
		if (date("Ym") == H_sess[HealthcareModView.PUB].cym) {
			A_tmp = {
				name: "pastflg",
	...
	//ここで使用する自作関数の読込
	{
		var A_rule = this.getHealthcareAddModFormRule();
	...
	{
		if (H_sess.post.healthid == H_sess.get.manageno && H_sess.post.healthcoid == H_sess.get.coid) {
			H_sess.post.pre_healthid = "";
			H_sess.post.pre_healthcoid = "";
			return false;
		} else {
			H_sess.post.pre_healthid = H_sess.get.manageno;
			H_sess.post.pre_healthcoid = H_sess.get.coid;
			return true;
		}
	}
	...
	viewChgKeyError() //エラー画面表示
	{
		...
		O_err.display("\u8ACB\u6C42\u304C\u3042\u308B\u30D8\u30EB\u30B9\u30B1\u30A2ID\u306E\u904B\u9001ID\u306F\u5909\u66F4\u3067\u304D\u307E\u305B\u3093\u3002", 0, _SERVER.PHP_SELF, "\u623B\u308B");
	}
	...
	//完了画面表示
	{
		...
		O_finish.displayFinish("\u30D8\u30EB\u30B9\u30B1\u30A2ID\u5909\u66F4", "/Management/Healthcare/menu.php", "\u4E00\u89A7\u753B\u9762\u3078");
	}
