...
//@return void
//
class ShopOrderListViewView extends OrderListMenuView {
	...
	//検索動作モード　trueなら「今日やるボタン」
	{
		...
		super(H_param);
		this.H_Dir = this.O_Sess.getPub(ShopOrderListViewView.PUB);
	...
	getLocalSession() {
		var H_sess = {
			[ShopOrderListViewView.PUB]: this.O_Sess.getPub(ShopOrderListViewView.PUB),
	...
	//getパラメータは消す
	{
		var A_sortcol = ["fromy", "toy", "seldate", "trans", "status", "ordertype", "misctype", "carid", "buyselid", "contractor", "orderid", "pactid", "telno", "sctcode", "pref", "deliverydate_type", "send_deliverydate_flg", "productname", "note", "is_open_misctype", "is_open_search2"];
		this.setDefaultSession();
		var sess = this.getLocalSession();

		if (!(undefined !== this.H_Dir.carid_only)) {
			switch (_SERVER.PHP_SELF) {
				case "/Shop/MTOrder/order_menu.php":
					this.H_Dir.carid_only = 1;
					break;

				default:
					this.H_Dir.carid_only = 0;
					break;
			}
		} else //ドコモ監査用URLの際には毎度いれるようにしよう・・
			//ドコモ監査用URLに直接リンクされた時の対応
			{
				if (_SERVER.PHP_SELF == "/Shop/MTOrder/order_menu.php") {
					this.H_Dir.carid_only = 1;
				}
			}
		...
		if (undefined !== _POST.search == true) {
			for (var val of Object.values(A_sortcol)) {
				if (true == (undefined !== _POST[val])) {
					...
					} else if (-1 !== ["orderid"].indexOf(val)) {
						this.H_Dir[val] = _POST[val].replace(/[^0-9],/g, "");
					} else if (-1 !== ["pactid"].indexOf(val)) {
						this.H_Dir[val] = _POST[val].replace(/[^0-9],/g, "");
					} else {
		...
		if (!(undefined !== this.H_Dir.is_open_misctype)) {
			this.H_Dir.is_open_misctype = "none";
		}

		if (!(undefined !== this.H_Dir.is_open_search2)) {
			this.H_Dir.is_open_search2 = "none";
		}

		this.O_Sess.setPub(ShopOrderListViewView.PUB, this.H_Dir);
		this.O_Sess.setSelfAll(this.H_Local);
	...
	makeSearchForm(H_box, H_select, carid_only = 0) //DBから取って来れない初期データを作る
	//フォーム要素の配列作成
	//統括部署なら部門コード検索させる
	//ordertype[9]はその他のID・・なので、その他のIDがもしも変わる事があれば、変えること
	//k76
	//k76
	//クイックフォームオブジェクト生成
	{
		...
		var A_form = [{
			...
			inputtype: "button",
			options: {
				onClick: "offToCheckbox('ordertype'," + H_box.ordertype.length + ",callbackOrderType)"
			}
		}, {
			name: "caronbtn",
			label: "\u5168\u3066\u9078\u629E",
			...
			name: "off_misctype_btn",
			label: "\u9078\u629E\u89E3\u9664",
			inputtype: "button",
			options: {
				onClick: "offToCheckbox('misctype'," + H_box.misctype.length + ")"
			}
		}, {
			name: "search",
			label: "\u691C\u7D22",
		...
		if (carid_only == 0) {
			A_car = this.makeSearchFormCheckBox(H_box.carid, "carid", "name");
		} else {
			A_car = Array();
			A_car.push({
				name: "carid[0]",
		...
		var A_misctype = this.makeSearchFormCheckBox(H_box.misctype, "misctype", "name", {
			onclick: "checkToCheckbox(\"ordertype[9]\")"
		});
		...
		A_form = array_merge(A_form, A_status, A_car, A_type, A_buy, A_rec, A_deliverydate_type, A_misctype, A_send_deliverydate_flg);
	...
	setSearchFormDefault(H_sess, H_date, carid_only = 0) //今日やるボタンの日付が指定されていたら引き継ぐ
	{
		...
		if (undefined == H_sess.post.post) {
			this.setDefaultSearch(H_date, carid_only);
		} else {
	...
	setDefaultSearch(H_date, carid_only = 0) //ドコモ監査用の処理いれた
	//carid_onlyは対象のキャリアが入ってるよ
	{
		this.H_def_form = {
			expectexec: {
				Y: H_date.y,
		...
		if (carid_only == 0) {
			this.H_def_form["carid[0]"] = "1";
			this.H_def_form["carid[1]"] = "1";
			this.H_def_form["carid[2]"] = "1";
			this.H_def_form["carid[3]"] = "1";
			this.H_def_form["carid[4]"] = "1";
			this.H_def_form["carid[5]"] = "1";
			this.H_def_form["carid[6]"] = "1";
		} else {
			this.H_def_form["carid[0]"] = "1";
		}
	}

	setDefaultSearchSession(H_sess, H_date, carid_only = 0) {
	...
	makeSearchFormCheckBox(H_box, name, label, options: ?{} | any[] = undefined) {
		var idx = 0;
		var A_form = Array();

		for (var val of Object.values(H_box)) //option追加
		{
			var opt = {
				id: name + "[" + idx + "]"
			};
	...
	clearUnderSession() {
		this.clearLastForm();
		var A_exc = [ShopOrderListViewView.PUB, "/Shop/Order/order_detail.php"];
	...
	setSearchWhere(H_sql) {
		...
		temp[ShopOrderListViewView.PUB].down.where = H_sql.where;
		temp[ShopOrderListViewView.PUB].down.orderby = H_sql.orderby;
		this.O_Sess.setPub(ShopOrderListViewView.PUB, temp[ShopOrderListViewView.PUB]);
	}
	...
	//assaign
	{
		...
		try {
			a.loadConfig(ShopOrderListViewView.INI_NAME);
		...
		this.get_Smarty().assign("is_open_search2", this.H_Dir.is_open_search2);
		this.get_Smarty().assign("is_open_misctype", this.H_Dir.is_open_misctype);

		if (_SERVER.REMOTE_ADDR == "61.192.221.92" || _SERVER.REMOTE_ADDR == "192.168.99.57") {
			this.get_Smarty().assign("hiddenLink", "on");
		}

		this.get_Smarty().display(KCS_DIR + "/template/Shop/MTOrder/rapid.tpl");
	}
