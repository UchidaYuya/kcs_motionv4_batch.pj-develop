...
//@return void
//
class AttachFilesOrderListView extends AttachFilesView {
	static UP_DIR = "order";
	static ORDER_KEY = "/MTOrder";
	static ATTACH_INFO = "attach_order.txt";
	...
	_getDir() {
		if (is_null(this._dir)) {
			var orderdata = this.gSess().getPub(AttachFilesOrderListView.ORDER_KEY);
	...
	_getFileinfo() {
		if (is_null(this._fileInfo)) {
			var filepath = this._getDir() + "/" + AttachFilesOrderListView.ATTACH_INFO;
	...
	_setUpableFileinfo() {
		this._makeDir(this._getDir() + "/" + AttachFilesOrderListView.UP_DIR);
		...
		if (!is_null(fileinfo)) {
			for (var key in fileinfo) {
				...
				if (!file_exists(this._getDir() + "/" + AttachFilesOrderListView.UP_DIR + "/" + key)) {
	...
	{
		if (undefined !== _GET.o) {
			this.gSess().setPub(AttachFilesOrderListView.ORDER_KEY, {
				orderid: +_GET.o
			});
		}
		...
		if (0 < _POST.length) {
			if (0 < _FILES.length) {
				for (var key in _FILES) {
					...
					var f_name = this._getDir() + "/order/" + this._getNextFilename();
