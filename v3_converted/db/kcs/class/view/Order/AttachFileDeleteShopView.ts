...
//@return void
//
class AttachFileDeleteShopView extends ViewSmarty {
	...
	checkCGIParam() {
		if (undefined !== _POST.filenumber && undefined !== _POST.type) {
			var orderdata = this.gSess().getPub(AttachFileDeleteShopView.ORDER_KEY);
			var filebasedir = KCS_DIR + "/files/OrderFiles/" + orderdata.orderid + "/";

			if (file_exists(KCS_DIR + "/files/OrderFiles/" + orderdata.orderid + "/")) {
