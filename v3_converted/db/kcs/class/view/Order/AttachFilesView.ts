...
class AttachFilesView extends ViewSmarty {
	...
	_attachDeleteDecision(orderid, dir) {
		var list = this._checkDirs + [AttachFilesView.HIDDEN];

		for (var name of Object.values(list)) {
			var temp = dir + "/" + name;
			...
			while (false !== (file = readdir(fh))) {
				if (file != "." && file != "..") {
					return this.getModel().signedAttachDelete(orderid, false);
				}
			}
		}

		return this.getModel().signedAttachDelete(orderid);
	}

	attachDeleteDecision(orderid, dir) {
		return this._attachDeleteDecision(orderid, dir);
	}
