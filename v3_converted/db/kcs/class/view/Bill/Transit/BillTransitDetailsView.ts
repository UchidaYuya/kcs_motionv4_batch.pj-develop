...
//@return void
//
class BillTransitDetailsView extends BillDetailsViewBase {
	constructor() {
		super();
	}
	...
	checkCGIParamPeculiar() //初期アクセス時ＩＤをセッションに入れリロード
	{
		if (undefined !== _GET.id == true) {
			...
			this.H_Local.get.tranid = A_id[0];
			this.H_Local.get.trancoid = A_id[1];
	...
	//英語化 20090702miya
	{
		var year = H_sess[BillTransitDetailsView.PUB].cym.substr(0, 4);
		var month = H_sess[BillTransitDetailsView.PUB].cym.substr(4, 2);
		var O_auth = MtAuthority.singleton(this.O_Sess.pactid);
		createTabBill5(this.get_Smarty(), year, month, H_sess[BillTransitDetailsView.PUB].current_postid, false, "transit", O_auth, this.O_Sess.userid);
		this.get_Smarty().assign("mode", H_sess.SELF.mode);
		this.get_Smarty().assign("tranid", H_sess.SELF.get.tranid);
		var A_co = O_model.getTranCoData(this.O_Sess.language);
		this.get_Smarty().assign("tranconame", A_co[H_sess.SELF.get.trancoid]);
	...
	makePankuzuLinkHash() //英語化 20090702miya
	{
		if ("ENG" == this.O_Sess.language) {
			var H_link = {
				"/Bill/Transit/menu.php": "Bill information",
				"": "Billing statement"
			};
		} else {
			H_link = {
				"/Bill/Transit/menu.php": "\u8ACB\u6C42\u60C5\u5831",
				"": "\u8ACB\u6C42\u660E\u7D30"
			};
		}
