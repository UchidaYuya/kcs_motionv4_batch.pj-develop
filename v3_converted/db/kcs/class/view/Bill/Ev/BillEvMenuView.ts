...
//@return void
//
class BillEvMenuView extends BillMenuViewBase {
	constructor() {
		super();
	}

	setDefaultSessionPeculiar() //EVキャリアがセッションに無ければ作る
	{
		if (undefined !== this.H_Local.evcoid === false) {
			this.H_Local.evcoid = "0";
		}
	}

	checkCGIParamPeculiar() //EVキャリアが変更された時
	{
		if (undefined !== _POST.evcoid === true) {
			this.H_Local.evcoid = _POST.evcoid;
		}
	}
	...
	//クイックフォームオブジェクト生成
	{
		var H_co = O_model.getBillEvCoData(H_sess);
	...
	//再計算中の画面表示
	{
		var year = H_sess[BillEvMenuView.PUB].cym.substr(0, 4);
		var month = H_sess[BillEvMenuView.PUB].cym.substr(4, 2);
		...
		createTabBill5(this.get_Smarty(), year, month, H_sess[BillEvMenuView.PUB].current_postid, false, "ev", O_auth, this.O_Sess.userid);
		applyRecalc(O_smarty, O_renderer, year, month, H_sess.SELF.evcoid, "ev", auth_recalc, false, "EV");
		...
		if (-1 !== A_auth.indexOf("fnc_ev_manage_vw") === true) {
			manage_detail = true;
		}
		...
		if (O_model.checkInRecalc(year, month) === true) {
			this.get_Smarty().display("ev_bill_underconst.tpl");
		} else {
			this.get_Smarty().display(this.getDefaultTemplate());
		}
	}

	makeDefaultValuePeculiar(H_sess, H_default) //EVキャリア
	{
		H_default.evcoid = H_sess.SELF.evcoid;
	}
