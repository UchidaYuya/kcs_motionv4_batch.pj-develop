...
//@return void
//
class ShopMnglogView extends ViewSmarty {
	...
	//呼び出し元
	{
		...
		if (undefined !== this.H_Local.limit == false) //クッキーに表示件数があればそれを使う
			{
				if (undefined !== _COOKIE.shopmnglog_limit == true) {
					this.H_Local.limit = _COOKIE.shopmnglog_limit;
				} else {
		...
		this.H_Local.groupid = this.O_Sess.groupid;
		this.H_Local.shopid = this.O_Sess.shopid;
		this.H_Local.mode = "shop";
	}
			...
			if (is_numeric(_POST.limit) == true && _POST.limit > 0 && preg_match("/\\./", _POST.limit) == false) //クッキーセット
				{
					...
					setcookie("shopmnglog_limit", _POST.limit, mktime(0, 0, 0, 12, 31, 2020));
				} else {
				this.H_Local.limit = 10;
