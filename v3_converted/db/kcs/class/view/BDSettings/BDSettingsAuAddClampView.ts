...
//@return void
//
class BDSettingsAuAddClampView extends BDSettingsDocomoAddClampView {
	...
	makeFormElements(H_str, H_data) {
		...
		if (!is_null(H_SessPost)) {
			this.O_Qf.setDefaultsWrapper({
				code: stripslashes(H_SessPost.code)
			});
			this.O_Qf.setDefaultsWrapper({
			...
			this.O_Qf.setDefaultsWrapper({
				key_file: stripslashes(H_SessPost.key_file)
			});
			this.O_Qf.setDefaultsWrapper({
		...
		this.A_rules = [{
			...
			name: "code",
			mess: H_str.code + "\u306F\u534A\u89D2\u82F1\u6570\u3067\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044",
			type: "regex",
			format: "/[a-zA-Z0-9]/",
			validation: "client"
		}, {
	...
	_addRules(H_str) {
		this.A_rules.push({
			...
			mess: H_str.key_pass + "\u3068\u78BA\u8A8D\u7528" + H_str.key_passcheck + "\u304C\u7570\u306A\u308A\u307E\u3059",
			type: "compare",
			format: undefined,
			validation: "client"
		});
