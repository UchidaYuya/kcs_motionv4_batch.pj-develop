...
if (lock(true, dbh) == false) {
	...
	logh.putError(G_SCRIPT_ERROR, "import_hand.php \u624B\u5165\u529B\u8ACB\u6C42\u30C7\u30FC\u30BF\u53D6\u8FBC\u51E6\u7406  \u65E2\u306B\u8D77\u52D5\u3057\u3066\u3044\u307E\u3059");
	throw die(1);
}
...
//会社単位に終了ログを出力
{
	...
	var TEL_result = dbh.getCol("select telno from " + telX_tb + " where pactid = " + dbh.escape(A_pactid[cnt]) + " ;", true);
	...
	//レコード毎の処理
	{
		...
		//ASP利用料がONの場合
		{
			...
			if (-1 !== TEL_result.indexOf(readbuff[3]) == false) //エリアコードと種別コードにデフォルト値をセット
				//tel_tbの存在チェック
				{
					switch (readbuff[2]) {
						case 1:
						...
						default:
							if (DEBUG_FLG) logging("WARNING: \u4E88\u5B9A\u5916\u306E\u30AD\u30E3\u30EA\u30A2\u30B3\u30FC\u30C9\u3067\u3059 " + infilename);
			...
			for (var h_code in H_hand_kamoku) {
				...
				if (readbuff[idx] != "") {
					copy_buf += A_pactid[cnt] + "\t" + readbuff[3] + "\t" + h_code + "\t" + h_name + "\t" + readbuff[idx] + "\t" + viewcnt + "\t" + nowtime + "\t" + readbuff[2] + "\n";
					viewcnt++;
				}
			...
			if (aspFlg == true) //ASP使用料を調べる
				//合計行のために１つ表示順を空ける
				{
					if (undefined !== asp_tbl[readbuff[2]] == true) //ダミー電話番号のときはASP使用料をカウントしない（0にする）ように分岐を追加 20071009miya
						...
						if (DEBUG_FLG) logging("WARNING: ASP\u4F7F\u7528\u6599\u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093 " + infilename);
					...
					if (asp_is_counted == true) {
						viewcnt++;
						copy_buf += A_pactid[cnt] + "\t" + readbuff[3] + "\t" + G_CODE_ASP + "\t" + "ASP\u4F7F\u7528\u6599" + "\t" + asp_bill + "\t" + viewcnt + "\t" + nowtime + "\t" + readbuff[2] + "\n";
						viewcnt++;
						copy_buf += A_pactid[cnt] + "\t" + readbuff[3] + "\t" + G_CODE_ASX + "\t" + "ASP\u4F7F\u7528\u6599\u6D88\u8CBB\u7A0E" + "\t" + asp_tax + "\t" + viewcnt + "\t" + nowtime + "\t" + readbuff[2] + "\n";
					}
				}
...
if (filesize(teldetail_filename) != 0) //tel_details_X_tb へインポート
	//インポート失敗した場合
	{
		var teldetailX_col = ["pactid", "telno", "code", "codename", "charge", "detailno", "recdate", "carid"];
