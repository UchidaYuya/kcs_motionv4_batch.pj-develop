...
//会社合計レコード
//税区分コード表
//$H_Zei_CODE = array( '10'=>"内税", '11'=>"合算", '13'=>"非対象等", '15'=>"個別", '80'=>"", '90'=>"", '00'=>"" );
//---------------------------------------------------------------------------
//共通ログファイル名
//ログListener を作成
//ログファイル名、ログ出力タイプを設定
...
var Meisai_POS = [6, 10, 11, 13, 15, 17, 27, 33, 34, 35, 47, 51, 55, 59, 63, 67, 71, 126, 136, 141, 162, 183, 204, 225, 246, 267, 288, 309, 330, 351, 372, 393, 414, 435, 456, 477, 498];
var Seikyu_POS = [2, 6, 16, 19];
...
//// 要望により停止
//	$file_tel = $dataDir ."/". "tel_tb" . $billdate . $pactid . FILE_EXT . ".tmp";
//	$fp_tel = fopen( $file_tel, "w" );
//	if( $fp_tel == null ){
//		$logh->putError(G_SCRIPT_ERROR, SCRIPT_NAMEJ . LOG_DELIM . $pactid . LOG_DELIM . LOG_DELIM . $billdate . LOG_DELIM
//			. $file_tel . "の書き込みオープン失敗." );
//		return 1 ;
//	}
//tel_XX_tb に追加する電話を出力
//// 要望により停止
//	foreach( $A_telData as $A_data ){
//		$rootPostid = $A_data[0];
//		$telno = $A_data[1];
//		fwrite($fp_tel,
//				$pactid. "\t" .
...
//	}
{
	...
	for (var A_data of Object.values(A_telXData)) {
		...
		if (1 > dbh.getOne(orgtelsql)) {
			...
			telInfo = dbh.getHash(telsql);
			var ordCount = dbh.getOne(orgtelsql);

			if (undefined !== telInfo[0]) {
				{
...
//最後の電話についてASP使用料を表示する.
{
	...
	//carid キャリアID
	{
		if (data[0] != telno && telno != "") //ASP利用料表示する
			{
				...
				if (aspFlg == true) //合計用に１つ進める
					...
					{
						detailNo++;
						fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + G_CODE_ASP + "\t" + H_utiwake[G_CODE_ASP].name + "\t" + aspCharge + "\t" + "\\N\t" + detailNo + "\t" + nowtime + "\t" + NTTCOM_CARRIER_ID + "\t" + "\n");
						detailNo++;
						fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + G_CODE_ASX + "\t" + H_utiwake[G_CODE_ASX].name + "\t" + +(aspCharge * G_EXCISE_RATE + "\t" + "\\N\t" + detailNo + "\t" + nowtime + "\t" + NTTCOM_CARRIER_ID + "\t" + "\n"));
					}
		...
		fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + ut_code + "\t" + ut_name + "\t" + data[2] + "\t" + H_Zei_CODE[data[3]] + "\t" + detailNo + "\t" + nowtime + "\t" + NTTCOM_CARRIER_ID + "\t" + data[4] + "\n");
	}
