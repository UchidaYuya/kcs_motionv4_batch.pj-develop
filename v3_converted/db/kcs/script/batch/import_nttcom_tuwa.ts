...
//エラーの標準出力 2004.12.02 by 上杉顕一郎
//相対パスから絶対パスへ変更 2004.12.02 by 上杉顕一郎
//
//===========================================================================
//UPDATE 2004/12/2 上杉顕一郎：相対パスから絶対パスへ変更
//定義
//プロセス解説文字列(日本語)
//プロセス解説文字列
//処理名称
...
//国際電話
//---------------------------------------------------------------------------
//機能：クランプファイルインポートプロセス(ＮＴＴコミュニケーションズ)
//UPDATE 2004/12/2 上杉顕一郎：エラーログを標準出力する
//UPDATE 2004/12/2 上杉顕一郎：エラーログを標準出力する
//エラー出力用ハンドル
//プロセスクラスのインスタンス化
//パラメータチェック
...
const G_PROCNAME_J_IMPORT = "\u901A\u8A71\u8A73\u7D30\u30A4\u30F3\u30DD\u30FC\u30C8\u30D7\u30ED\u30BB\u30B9[NTT\u30B3\u30DF\u30E5\u30CB\u30B1\u30FC\u30B7\u30E7\u30F3\u30BA]";
const G_PROCNAME_IMPORT = "import_ntt_com";
const G_PROCNAME = "import_ntt_com";
const G_COMPANY_ID = "NTT_com/tuwa";
const G_CARRIER_ID = "9";
const G_RECODE_ID = "3";
const G_WORLD_SERVICE = "30";

//ＮＴＴ西日本prime
//一行(固定長)を分割して処理する
class import_ntt_com extends import_ntt_tuwa_base {
	doDivisionLine(line, ins_comm, a_bill_prtel) //レコード種別:G_RECODE_IDのみ
	{
		...
		if (recode_id == G_RECODE_ID) //課金先電話番号
			...
			//for DEBUG
			{
				...
				var serviceCode = line.substr(28, 2);
				...
				if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
					var ken_code = line.substr(70, 2);
					var ken_name = this.m_h_wld_area[ken_code];
				} else {
					ken_code = line.substr(69 - 1, 2);
				...
				{
					if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
						var tuwa_time = line.substr(72, 8);
					} else {
						tuwa_time = line.substr(71 - 1, 8);
					}
				...
				{
					if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
						var tuwa_money = line.substr(80, 10);
					} else {
						tuwa_money = line.substr(79 - 1, 10);
					}
