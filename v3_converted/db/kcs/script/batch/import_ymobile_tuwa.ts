...
const SCRIPT_NAMEJ = "ymobile\u901A\u8A71\u660E\u7D30\u30D5\u30A1\u30A4\u30EB\u30A4\u30F3\u30DD\u30FC\u30C8";
const SCRIPTNAME = "import_ymobile_tuwa.php";
const FIN_DIR = "fin";
const LOG_DELIM = " ";
...
class VodaProcessBase {
	...
	//処理すべきファイル種別の番号
	{
		this.SCRIPT_FILENAME = "import_ymobile_tuwa.php";
	...
	//ここまでに成功したファイルが無ければ終了する.
	{
		...
		this.dataDir = DATA_DIR + "/" + this.billdate + this.source_dir;
		...
		//正常処理が終わった pactid のみ書き出し処理
		{
			...
			var A_codes = Array();
			sql = "select prtelno from bill_prtel_tb where pactid=" + pactid + " and carid=" + this.carid;
			...
			if (this.EnableKousi == true) //kousiflg = 2:未登録 の電話はマスターから削除することになるので拾わない
				...
				//公私フラグの初期化
				{
					sql = "select telno, totelno, kousiflg from kousi_totel_master_tb where pactid=" + pactid + " and carid=" + this.carid + " and kousiflg != '2' order by telno, totelno";
					H_result = this.dbh.getHash(sql, true);
		...
		if (errFlag == true) //請求先番号が存在しない場合のメール送信予約追加20101202morihara
			{
				...
				var sql = "insert into clamp_error_tb" + "(pactid,carid,error_type,message,is_send,recdate,fixdate)" + "values" + "(" + pactid + "," + this.carid + ",'prtelno'" + ",'\u30BD\u30D5\u30C8\u30D0\u30F3\u30AF\u306E\u8ACB\u6C42\u5148\u756A\u53F7\u304C\u767B\u9332\u3055\u308C\u3066\u3044\u308B\u89AA\u756A\u53F7\u3068\u7570\u306A\u3063\u3066\u3044\u307E\u3059'" + ",false" + ",'" + date("Y/m/d H:i:s") + "'" + ",'" + date("Y/m/d H:i:s") + "'" + ")" + ";";
				this.dbh.query(sql);
				return 1;
			}
	...
	//明細件数カウント.
	{
		...
		var fromplace = "\\N";
		var time = record[6].trim();
		var charge = +record[11];
		var kousiflg = this.getTelKousi(pactid, telno, totelno);
		this.A_commFileData.push(pactid + "\t" + telno + "\t" + this.TEL_TYPE + "\t" + date + "\t" + totelno + "\t" + toplace + "\t" + fromplace + "\t" + time + "\t" + charge + "\t" + "\\N\t" + "\\N\t" + this.carid + "\t" + kousiflg + "\n");
		sum += charge;
		this.ReadMeisaiCnt++;
		return 0;
	}
	...
	//明細の処理
	{
		...
		//公私フラグ
		{
			...
			this.A_commFileData.push(pactid + "\t" + H_packet.telno + "\t" + this.TEL_TYPE + "\t" + H_packet.date + "\t" + totelno + "\t" + "\\N\t" + "\\N\t" + "\\N\t" + charge + "\t" + H_packet.byte + "\t" + H_packet.chargeseg + "\t" + this.carid + "\t" + kousiflg + "\n");
			sum += charge;
		}
	...
	//明細件数カウント.
	{
		...
		this.A_infoFileData.push(pactid + "\t" + telno + "\t" + sitename + "\t" + accounting + "\t" + date + "\t" + charge + "\t" + this.carid + "\n");
		sum += charge;
		this.ReadMeisaiCnt++;
		return 0;
	}
	...
	//明細件数カウント.
	{
		...
		var bytes = +record[7];
		var kousiflg = this.getTelKousi(pactid, telno, totelno);
		this.A_commFileData.push(pactid + "\t" + telno + "\t" + this.TEL_TYPE + "\t" + date + "\t" + totelno + "\t" + toplace + "\t" + fromplace + "\t" + time + "\t" + charge + "\t" + bytes + "\t" + "\\N\t" + this.carid + "\t" + kousiflg + "\n");
		sum += charge;
		this.ReadMeisaiCnt++;
		return 0;
	}
	...
	//'-'ハイフンを除く
	{
		...
		if (undefined !== this.H_TelKousi[telno]) //公私分計しないのであれば、そこで終了
			...
			{
				var sql = "select kousiptn, COALESCE(kousiflg,'(null)') as nkflag from tel_tb where pactid=" + pactid + " and carid=" + this.carid + " and telno='" + telno + "'";
				var H_result = this.dbh.getHash(sql, true);
				...
				if (undefined !== tel_kousiflg == false || tel_kousiflg == "(null)") //print "DEBUG_02: default_kousiflg=". $default_kousiflg .", and  pattern_id=". $pattern_id ."\n";
					...
					{
						sql = "select kousiflg, patternid from kousi_default_tb where pactid=" + pactid + " and carid=" + this.carid;
						H_result = this.dbh.getHash(sql, true);
				...
				if (tel_kousiflg == 0) //kousi_pattern_tb からフラグを取得
					...
					{
						sql = "select comhistflg, comhistbaseflg from kousi_pattern_tb where patternid=" + pattern_id + " and carid=" + this.carid;
						H_result = this.dbh.getHash(sql, true);
		...
		if (totelno != "" && undefined !== this.H_totel_master[telno][totelno]) //あれば、マスターの値を用いる
			...
			} else //print "DEBUG_06: comhistbaseflg base, kousiflg=". $kousiflg ."\n";
			{
				if (totelno != "") //「未登録」で kousi_totel_master に電話を登録する。
					...
					{
						this.A_totelFileData.push(pactid + "\t" + telno + "\t" + this.carid + "\t" + totelno + "\t" + "2" + "\n");
	...
	//delete失敗した場合
	{
		var sql_str = "delete from " + this.commhistory_tb + " where pactid in (" + A_pactDone.join(",") + ") and carid = " + this.carid;
		...
		sql_str = "delete from " + this.infohistory_tb + " where pactid in (" + A_pactDone.join(",") + ") and carid = " + this.carid;
		...
		sql_str = "delete from kousi_totel_master_tb where pactid in (" + A_pactDone.join(",") + ") and carid = " + this.carid + " and kousiflg = '2'";
		rtn = db.query(sql_str, false);
	...
	lock(is_lock) //ロックする
	{
		...
		if (is_lock == true) //２重起動防止チェックの不具合修正 2009/3/19 s.maeda
			...
			//ロック解除
			{
				...
				var nowtime = VodaProcessBase.getTimestamp();
