...
//会社単位に終了ログを出力
{
	...
	//ファイルが開けなかった時
	{
		...
		//通行料金
		{
			...
			var trg_post = dbh.getOne("select postid from card_bill_master_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and card_master_no = '" + A_line[11] + "' and cardcoid = " + A_line[1] + ";", true);
			var aspFlg = false;
			...
			if (A_line[4] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(TSUKOU_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + TSUKOU_UTICODE);
			...
			if (A_line[5] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(CAR_KOUSOKU_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CAR_KOUSOKU_UTICODE);
			...
			if (A_line[6] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(CARDIS_KOUSOKU_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CARDIS_KOUSOKU_UTICODE);
						logh.putError(G_SCRIPT_WARNING, SCRIPT_FILENAME + "WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CARDIS_KOUSOKU_UTICODE);
						error_flg = true;
						break;
					}

					write_buf += A_pactid[cnt] + "\t" + A_line[2] + "\t" + CARDIS_KOUSOKU_UTICODE + "\t\u8ECA\u4E21\u5358\u4F4D\u5272\u5F15\u984D\uFF08\u9AD8\u901F\u9053\u8DEF\uFF09\t" + A_line[6] * -1 + "\t\t" + viewcnt + "\t" + nowtime + "\t" + A_line[1] + "\t" + A_line[11] + "\n";
					viewcnt++;
				}
			...
			if (A_line[7] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(CAR_IPPAN_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CAR_IPPAN_UTICODE);
			...
			if (A_line[8] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(CARDIS_IPPAN_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CARDIS_IPPAN_UTICODE);
						logh.putError(G_SCRIPT_WARNING, SCRIPT_FILENAME + "WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CARDIS_IPPAN_UTICODE);
						error_flg = true;
						break;
					}

					write_buf += A_pactid[cnt] + "\t" + A_line[2] + "\t" + CARDIS_IPPAN_UTICODE + "\t\u8ECA\u4E21\u5358\u4F4D\u5272\u5F15\u984D\uFF08\u4E00\u822C\u6709\u6599\u9053\u8DEF\uFF09\t" + A_line[8] * -1 + "\t\t" + viewcnt + "\t" + nowtime + "\t" + A_line[1] + "\t" + A_line[11] + "\n";
					viewcnt++;
				}
			...
			if (A_line[9] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(CHOUSEI_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + CHOUSEI_UTICODE);
			...
			if (A_line[10] != "") //内訳コード正当性チェック
				{
					if (-1 !== A_uticode[A_line[1]].indexOf(SEIKYU_UTICODE) == false) {
						if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + SEIKYU_UTICODE);
			...
			if (-1 !== A_uticode[A_line[1]].indexOf(G_CODE_ASPX) == false) {
				if (DEBUG_FLG) logging("WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + G_CODE_ASPX);
				logh.putError(G_SCRIPT_WARNING, SCRIPT_FILENAME + "WARNING: \u672A\u8A2D\u5B9A\u306E\u5185\u8A33\u30B3\u30FC\u30C9\u3067\u3059 cardcoid:" + A_line[1] + "  \u5185\u8A33\u30B3\u30FC\u30C9:" + G_CODE_ASPX);
				error_flg = true;
				break;
			}

			if (aspFlg == true && preg_match("/" + A_pactid[cnt] + "\t" + A_line[2] + "\t" + G_CODE_ASPX + "\tASP\u4F7F\u7528\u6599.*\t" + A_line[1] + "\t" + A_line[11] + "\n/", write_buf) == false) //合計行のために１つ表示順を空ける
				{
					write_buf += A_pactid[cnt] + "\t" + A_line[2] + "\t" + G_CODE_ASPX + "\tASP\u4F7F\u7528\u6599\t" + asp_charge + "\t\t100\t" + nowtime + "\t" + A_line[1] + "\t" + A_line[11] + "\n";
				}

			if (-1 !== CARD_result.indexOf(A_line[2]) == false && preg_match("/" + A_line[2] + "/", card_xx_write_buf) == false) //カードの種類がコーポレート
				//そのカード会社のお客様番号（法人番号）の登録がなかったら（登録先部署取得失敗）ルート部署を取る
				{
					...
					if (trg_post == "") //登録先部署はルート部署
						{
							if (DEBUG_FLG) logging("ERROR: \u672A\u767B\u9332\u30AB\u30FC\u30C9\u306E\u767B\u9332\u5148\u90E8\u7F72\uFF08\u304A\u5BA2\u69D8\u756A\u53F7\u306E\u767B\u9332\u304C\u3042\u308B\u90E8\u7F72\uFF09\u304C\u53D6\u5F97\u3067\u304D\u306A\u304B\u3063\u305F\u306E\u3067\u30EB\u30FC\u30C8\u90E8\u7F72\u306B\u767B\u9332\u3057\u307E\u3059" + "select postid from card_bill_master_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and card_master_no = '" + A_line[11] + "' and cardcoid = " + A_line[1] + ";");
							...
							if (preg_match("/(\\D)/", trg_post) == true) {
								if (DEBUG_FLG) logging("ERROR: \u672A\u767B\u9332\u30AB\u30FC\u30C9\u306E\u767B\u9332\u5148\u90E8\u7F72\u304C\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093" + "select postid from card_bill_master_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and card_master_no = '" + A_line[11] + "' and cardcoid = " + coid + ";");
			...
			if (target == "n") //card_tbにデータが無いときはコピー文作成
				{
					if (-1 !== CARD_now_result.indexOf(A_line[2]) == false && preg_match("/" + A_line[2] + "/", card_write_buf) == false) //カードの種類がコーポレート
						//そのカード会社のお客様番号（法人番号）の登録がなかったら（登録先部署取得失敗）ルート部署を取る
						{
							if (A_line[1] <= 4) {
								trg_coid = 100;
		...
		if (rtn != 0) //ロールバック
			{
				...
				logh.putError(G_SCRIPT_ERROR, SCRIPT_FILENAME + " ETC\u624B\u5165\u529B\u8ACB\u6C42\u30C7\u30FC\u30BF\u53D6\u8FBC\u51E6\u7406 " + carddetailX_tb + "\u306E\u30C7\u30FC\u30BF\u30A4\u30F3\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F " + rtn.userinfo);
				dbh.rollback();
				throw die(1);
			} else {
			...
			while (copyfileName = readdir(dirh)) {
				if (is_file(dataDir + "/" + copyfileName) == true && preg_match("/CardHandInfo/i", copyfileName) == true) {
					if (rename(dataDir + "/" + copyfileName, finDir + "/" + copyfileName) == false) {
						...
						logh.putError(G_SCRIPT_ERROR, "import_etc_coprhist.phpETC\u624B\u5165\u529B\u8ACB\u6C42\u30C7\u30FC\u30BF\u53D6\u8FBC\u51E6\u7406  \u30D5\u30A1\u30A4\u30EB\u306E\u79FB\u52D5\u306B\u5931\u6557\u3057\u307E\u3057\u305F " + dataDir + "/" + copyfileName);
						throw die(1);
					} else {
...
function checkInfoFormat(A_line) //1,お客様番号
{
	...
	if (preg_match("/(\\D)/", A_line[0]) == true) {
		if (DEBUG_FLG) logging("\u5951\u7D04ID\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[0]);
		errcnt++;
	}

	if (preg_match("/(\\D)/", A_line[1]) == true) {
		if (DEBUG_FLG) logging("ETC\u30AB\u30FC\u30C9\u7A2E\u985E\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[1]);
		errcnt++;
	}
	...
	if (A_line[5] != "" && preg_match("/(\\D)/", A_line[5]) == true) {
		if (DEBUG_FLG) logging("\u5272\u5F15\u5BFE\u8C61\u984D\uFF08\u9AD8\u901F\u9053\u8DEF\uFF09\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[5]);
		errcnt++;
	}

	if (A_line[6] != "" && preg_match("/(\\D)/", A_line[6]) == true) {
		if (DEBUG_FLG) logging("\u5272\u5F15\u984D\uFF08\u9AD8\u901F\u9053\u8DEF\uFF09\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[6]);
		errcnt++;
	}

	if (A_line[7] != "" && preg_match("/(\\D)/", A_line[7]) == true) {
		if (DEBUG_FLG) logging("\u5272\u5F15\u5BFE\u8C61\u984D\uFF08\u4E00\u822C\u6709\u6599\u9053\u8DEF\uFF09\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[7]);
		errcnt++;
	}

	if (A_line[8] != "" && preg_match("/(\\D)/", A_line[8]) == true) {
		if (DEBUG_FLG) logging("\u5272\u5F15\u984D\uFF08\u4E00\u822C\u6709\u6599\u9053\u8DEF\uFF09\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[8]);
		errcnt++;
	}

	if (A_line[9] != "" && preg_match("/(\\D)/", A_line[9]) == true) {
		if (DEBUG_FLG) logging("\u8ABF\u6574\u91D1\u984D\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[9]);
		errcnt++;
	}

	if (A_line[10] != "" && preg_match("/(\\D)/", A_line[10]) == true) {
		if (DEBUG_FLG) logging("\u8ACB\u6C42\u91D1\u984D\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[10]);
		errcnt++;
	}

	if (preg_match("/(\\D)/", A_line[11]) == true) {
		if (DEBUG_FLG) logging("\u304A\u5BA2\u69D8\u756A\u53F7\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[11]);
		errcnt++;
	}
