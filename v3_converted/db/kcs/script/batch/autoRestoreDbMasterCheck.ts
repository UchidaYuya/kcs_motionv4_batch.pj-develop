...
if (file_exists(LOCAL_DIR + BACKUP_DATABASE + ".gz") == true) //postgres再起動(restartだとなぜかうまくいかなかった・・)
	//DBドロップ
	//DBクリエイト
	//DBレストア
	//バックアップデータがなかった場合(kcsv2)
	{
		exec("/usr/bin/sudo -u root  /etc/rc.d/init.d/postgres  stop", output);
		exec("/usr/bin/sudo -u root  /etc/rc.d/init.d/postgres  start", output);
		sleep(5);
