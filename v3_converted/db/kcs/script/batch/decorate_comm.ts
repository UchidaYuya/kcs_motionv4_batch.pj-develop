...
//array
//(
//"column" => tel_X_tbで、強調の有無を決定するカラム名(text型),
//"A_tel_key" => 上記のcolumnで、強調を行う値の配列(○や◎),
//"A_pactid" => tel_X_tbで参照するpactid,
//"A_comm_type" => array(
//キャリアID => array(commhistory_X_tbで、処理対象のtype),
//),
//);
//機能：顧客毎の処理を実行する
//引数：顧客ID
//作業ファイル保存先
//返値：深刻なエラーが発生したらfalseを返す
//機能：文字列をクォートで囲んでカンマでつないで返す
//引数：文字列の配列
//返値：変換した文字列
class ProcessDecorate extends ProcessDefault {
	...
	//全キャリアを読み出す
	{
		...
		var A = this.m_db.getAll(sql);

		for (var AA of Object.values(A)) {
			var carid = AA[0];
	...
	//全社共通と自社の設定を上書きする
	{
		...
		for (var key of Object.values(A_key)) //A_comm_type以外の設定があればコピーする
		//キャリア毎の設定があればコピーする
		{
	...
	executePactid(pactid, logpath) //この顧客に強調表示権限が無ければ何もしない
	...
	implode(A_text) {
		...
		for (var text of Object.values(A_text)) {
			if (!is_first) rval += ",";
			is_first = false;
