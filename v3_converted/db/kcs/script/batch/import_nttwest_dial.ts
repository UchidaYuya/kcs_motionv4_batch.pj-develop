...
//define("NET_KIND", 1);	// 網種別は"1"	// ダイアルインに網種別はない.
//NTT西日本の地域コード
//NTT西日本の回線ID
//define("NTTEAST_AREA_ID", 34 );	// NTT東日本の地域コード
//define("NTTEAST_CIRCUIT_ID", 17 );	// NTT東日本の回線ID
//ダイアルインを表す接尾子
//以下はcommonで定義済み
...
const SCRIPT_NAMEJ = "NTT\u897F\u65E5\u672C\u8ACB\u6C42\u30C0\u30A4\u30A2\u30EB\u30A4\u30F3\u30A4\u30F3\u30DD\u30FC\u30C8";
...
var sql = "select code,name,codetype,taxtype from utiwake_tb where carid=" + NTTWEST_CARRIER_ID;
...
//ここで合計金額チェックする.
{
	...
	if (chargeSum != sum) {
		logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + "\u3054\u8ACB\u6C42\u91D1\u984D\u304C\u5185\u8A33\u306E\u5408\u8A08\u306B\u4E00\u81F4\u3057\u307E\u305B\u3093.(" + chargeSum + "!=" + sum + ").");
		return 1;
	}
...
//tel_XX_tb に追加する電話を出力
{
	...
	for (var A_data of Object.values(A_telXData)) {
		rootPostid = A_data[0];
		var telno = A_data[1];
		fwrite(fp_telX, pactid + "\t" + rootPostid + "\t" + telno + "\t" + telno + "\t" + NTTWEST_CARRIER_ID + "\t" + NTTWEST_AREA_ID + "\t" + NTTWEST_CIRCUIT_ID + "\t" + nowtime + "\t" + nowtime + "\n");
	}

	for (var A_data of Object.values(A_telData)) {
		rootPostid = A_data[0];
		telno = A_data[1];
		fwrite(fp_tel, pactid + "\t" + rootPostid + "\t" + telno + "\t" + telno + "\t" + NTTWEST_CARRIER_ID + "\t" + NTTWEST_AREA_ID + "\t" + NTTWEST_CIRCUIT_ID + "\t" + nowtime + "\t" + nowtime + "\n");
	}
...
//最後の電話についてASP使用料を表示する.
{
	...
	//carid キャリアID
	{
		if (data[0] != telno && telno != "") //ASP利用料表示する
			{
				...
				if (aspFlg == true) //合計用に１つ進める
					...
					{
						detailNo++;
						fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + G_CODE_ASP + "\t" + H_utiwake[G_CODE_ASP].name + "\t" + aspCharge + "\t" + "\\N\t" + detailNo + "\t" + nowtime + "\t" + NTTWEST_CARRIER_ID + "\t" + "\n");
						detailNo++;
						fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + G_CODE_ASX + "\t" + H_utiwake[G_CODE_ASX].name + "\t" + +(aspCharge * G_EXCISE_RATE + "\t" + "\\N\t" + detailNo + "\t" + nowtime + "\t" + NTTWEST_CARRIER_ID + "\t" + "\n"));
					}
		...
		fwrite(fp_teldetails, pactid + "\t" + telno + "\t" + data[1] + "\t" + ut_name + "\t" + data[2] + "\t" + H_Zei_CODE[data[3]] + "\t" + detailNo + "\t" + nowtime + "\t" + NTTWEST_CARRIER_ID + "\t" + data[4] + "\n");
	}
