...
//@return void
//
class importNTTComBillModel extends importNTTComModelBase {
	static BILLDIR = "bill";
	...
	doEachFile(H_utiwake, filename, pactid) //請求部分
	{
		...
		while (line = fgets(fp)) {
			...
			if (importNTTComBillModel.KIND_BILL == datakind) {
				...
				var amountData = {
					year: this.H_argv.billdate.substr(0, 4),
					month: this.H_argv.billdate.substr(4, 2),
					pactid: pactid,
					carid: this.carid
				};
	...
	doImport() //20110516
	{
		...
		if (0 < filesize(this.O_tab.getFilePath())) {
			if (false == this.doCopyInsert(this.O_tab.getTableName(), this.O_tab.getFilePath(), this.O_tab.getAmountColumn())) {
	...
	execBillDetail(H_utiwake, H_code, line, pactid, filename) //内訳情報を取得する
	{
		...
		if (true != preg_match("/^0[789]0/", telno) && true == preg_match("/^00/", telno)) {
			telno = telno.substr(1, telno.length);
		}
		...
		{
			var indivbill = line.substr(i * importNTTComBillModel.BILL_LENGTH + 141, importNTTComBillModel.BILL_LENGTH);
	...
	setBillDir(carid = "") {
		...
		switch (carid) {
			case importNTTComBillModel.NTTCOM_CARID:
				this.billdir = importNTTComBillModel.BILLDIR;
				break;

			case importNTTComBillModel.NTTCOM_FIX_CARID:
	...
	writeTelData(pactid) //tel_x_tbに追加する電話を出力
	{
		...
		//公私分計をする電話は全て「会社設定にしたがう」にする
		{
			...
			fwrite(fp_telx, pactid + "\t" + postid + "\t" + telno + "\t" + telno_view + "\t" + H_tmp_info[0] + "\t" + this.carid + "\t" + this.arid + "\t" + this.cirid + "\t" + H_tmp_info[1] + "\t" + H_tmp_info[2] + "\t" + H_tmp_info[3] + "\t" + H_tmp_info[4] + "\t" + H_tmp_info[5] + "\t" + H_tmp_info[6] + "\t" + H_tmp_info[7] + "\t" + H_tmp_info[8] + "\t" + H_tmp_info[9] + "\t" + H_tmp_info[10] + "\t" + H_tmp_info[11] + "\t" + H_tmp_info[12] + "\t" + H_tmp_info[13] + "\t" + H_tmp_info[14] + "\t" + H_tmp_info[15] + "\t" + H_tmp_info[16] + "\t" + H_tmp_info[17] + "\t" + H_tmp_info[18] + "\t" + H_tmp_info[19] + "\t" + H_tmp_info[20] + "\t" + H_tmp_info[21] + "\t" + H_tmp_info[22] + "\t" + H_tmp_info[23] + "\t" + H_tmp_info[24] + "\t" + H_tmp_info[25] + "\t" + H_tmp_info[26] + "\t" + H_tmp_info[27] + "\t" + H_tmp_info[28] + "\t" + H_tmp_info[29] + "\t" + H_tmp_info[30] + "\t" + H_tmp_info[31] + "\t" + H_tmp_info[32] + "\t" + H_tmp_info[33] + "\t" + H_tmp_info[34] + "\t" + H_tmp_info[35] + "\t" + H_tmp_info[36] + "\t" + nowtime + "\t" + nowtime + "\n");
		}
	...
	//按分
	{
		...
		for (var val of Object.values(this.A_filedata)) {
			...
			fwrite(fp, pactid + "\t" + val[0] + "\t" + val[1] + "\t" + val[4] + "\t" + val[2] + "\t" + val[3] + "\t" + detailno + "\t" + nowtime + "\t" + this.carid + "\t" + this.billno + "\n");
			A_detail[val[0]] = 1;
		}
		...
		if ("y" == this.H_argv.anbun) {
			for (var val of Object.values(H_anbundata)) {
				...
				fwrite(fp, pactid + "\t" + val[0] + "\t" + val[1] + "\t" + val[5] + "\t" + val[2] + "\t" + "\u975E\u5BFE\u8C61\u7B49\t" + detailno + "\t" + nowtime + "\t" + this.carid + "\t" + this.billno + "\n");
				detailno++;
		...
		for (var val of Object.values(this.A_outtel)) {
			if (this.dummytelno != val) {
				...
				if (true == aspflg) {
					fwrite(fp, pactid + "\t" + val + "\t" + importNTTComBillModel.REF_ASP + "\t" + importNTTComBillModel.REF_ASP_NAME + "\t" + aspcharge + "\t" + "\\N\t" + detailno + "\t" + nowtime + "\t" + this.carid + "\t" + "\\N" + "\n");
					detailno++;
					fwrite(fp, pactid + "\t" + val + "\t" + importNTTComBillModel.REF_ASX + "\t" + importNTTComBillModel.REF_ASX_NAME + "\t" + +(aspcharge * importNTTComBillModel.EXCISE_RATE + "\t" + "\\N\t" + detailno + "\t" + nowtime + "\t" + this.carid + "\t" + "\\N" + "\n"));
				}
			}
		}
	...
	execMainProc(H_pact, H_utiwake, H_erace) //指定されたpact分ループ
	{
		...
		//削除してなければ実行
		{
			...
			if (Array() == A_billFile) {
				this.outWarning("pactid=" + pactid + "\u306E\u8ACB\u6C42\u30C7\u30FC\u30BF\u30D5\u30A1\u30A4\u30EB\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093");
				closedir(dirh);
				continue;
			} else {
			...
			this.billno = this.getBillNo(this.carid, pactid);
