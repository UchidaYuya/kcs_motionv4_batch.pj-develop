...
class UpdateRecomBase extends ScriptDBAdaptor {
	UpdateRecomBase(listener, db, table_no, inserter_recom, inserter_log, A_cirid, A_monthcnt, assert, carid) //プランに適用できるパケットIDの読み出し
	//プランマスターの読み出し
	//パケットマスターの読み出し
	//現行プラン情報
	//predataのうち、割引率を表すキー
	...
	//ファイルに書き出した値をDBにインポートする
	{
		...
		if (!this.m_O_inserter_recom.setConst(const)) {
		...
		if (!this.m_O_inserter_log.setConst(const)) {
	...
	do_execute_sim(year, month, O_fetch_tel, A_fetch, A_disratiotalk) //処理すべき電話番号を取り出す
	{
		var H_telno;
	...
	//使用頻度の高い電話
	{
		...
		for (var tgt in H_keys) {
			...
			for (var src of Object.values(A_src)) H_recom[tgt] += H_ave.predata[src];
