...
//現在時刻
//対象処理が最新かどうか
//SEO特別対応、使用者->カード名義を上書き
//ルート部署のpostid
//カード名義書き換え用、SUO特別対応時に使う
...
class ImportEtcCardBase {
	...
	//終了
	{
		...
		var A_para = ["-e", "-y", "-p", "-b", "-t", "-s"];

		if (_SERVER.argv.length != 7) //数が正しくない
			{
				this.usage("", this.dbh);
			} else //数が正しい
			//$cnt 0 はスクリプト名のため無視
				...
				for (var cnt = 1; cnt < argvCnt; cnt++) //mode を取得
				{
					...
					if (ereg("^-s", _SERVER.argv[cnt]) == true) {
						...
						if (ereg("^[ny]$", this.SUOspec) == false) {
							this.usage("\u4F7F\u7528\u8005\u306B\u3088\u308B\u30AB\u30FC\u30C9\u540D\u7FA9\u306E\u4E0A\u66F8\u304D\uFF08\u3042\u308A/\u306A\u3057\uFF09\u6307\u5B9A\u304C\u4E0D\u6B63\u3067\u3059\u3002" + _SERVER.argv[cnt]);
						}
		...
		if (String(this.SUOspec === "y")) {
			if (this.DEBUG) this.logging("INFO: SUO\u7279\u4F8B\u51E6\u7406\u3001\u30AB\u30FC\u30C9\u540D\u7FA9\u3067\u4F7F\u7528\u8005\u3092\u4E0A\u66F8\u304D\u3057\u307E\u3059");
			rtn = this.doSUOspec(cardX_tb);
		}
	...
	usage(comment) {
		...
		print("\n" + comment + "\n\n");
		print("Usage) " + _SERVER.argv[0] + " -e={O|A} -y=YYYYMM -p={PACTID} -b={Y|N} -t={N|O} -s={Y|N}\n");
		...
		print("\t\t-s \u30AB\u30FC\u30C9\u540D\u7FA9\u3092\u4F7F\u7528\u8005\u3067\t(Y:\u4E0A\u66F8\u304D\u3059\u308B,N:\u3057\u306A\u3044) \n\n");
		throw die(1);
	}
