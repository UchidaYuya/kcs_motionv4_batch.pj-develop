...
//プレフィックス("tel_tb."を想定)
//返値：ANDで始まるSQL文の一部
class updateTrendVoda extends ScriptDBAdaptor {
	UpdateTrendVoda(listener, db, table_no, inserterTrend, inserterToptel) {
	...
	executePacketRatio(pactid, carid, table_no) //DEBUG * ToDo *
	//いまのところ見分け方が解らない。。。
	//集計対象となるtypeを抜き出す
	//本来あってはならない未分類値
	//byteの有無で通話/通信を判断する
	...
	//tuwasecが作られなかった場合はtuwasec=0を追加
	{
		...
		while (line = result.fetchRow(DB_FETCHMODE_ASSOC)) {
			if (line.type_check == 1) //「国内通話」だけを追加する
				...
				//通常の通話
				{
					...
					if (is_null(line.value) == true) {
						line.value = 0;
					}

					var key = line.telno;

					if (strcmp(cur_key, key)) //電話番号が変わったら出力する
						{
							if (cur_key.length) {
		...
		//20110826morihara 自社携帯とそれ以外で按分
		{
			...
			if (is_null(charge) == true || charge == 0) //$this->putError(G_SCRIPT_WARNING,
