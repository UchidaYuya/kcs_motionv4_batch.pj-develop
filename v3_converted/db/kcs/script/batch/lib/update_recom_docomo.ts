...
class UpdateRecomDocomo extends UpdateRecomBase {
	UpdateRecomDocomo(listener, db, table_no, inserter_recom, inserter_log, A_cirid, A_monthcnt, assert) //統計情報平均値のデフォルト値
	//パケットパック無しを取り出す
	...
	//パケットパックマスターを拡張して、パケホーダイ関連の情報を入れる
	{
		...
		this.UpdateRecomBase(listener, db, table_no, inserter_recom, inserter_log, A_cirid, A_monthcnt, assert, G_CARRIER_DOCOMO);
...

		{
			...
			for (var cirid in _tmp_0) {
				...
				if (G_CIRCUIT_FOMA != cirid) continue;
