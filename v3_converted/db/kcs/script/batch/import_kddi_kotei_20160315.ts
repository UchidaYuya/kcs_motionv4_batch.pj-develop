						...
						//請求金額以外の場合
						{
							if (H_MargeData[kokyakuno][margeCounter][0] != oldTelno && oldTelno != "" && oldTelno != H_DummyTel[kokyakuno][0]) //消費税を求める
								...
								//初期化 デフォルトは追加する
								{
									...
									if (0 != tax) //消費税を出力
										{
											A_teldetailsOutputBuff.push(A_pactid[pactCounter] + "\t" + oldTelno + "\t" + TAX + "\t" + H_Code[TAX] + "\t" + tax + "\t\\N\t" + detailNo + "\t" + now + "\t" + G_CARRIER_KDDI_KOTEI + "\t\n");
											detailNo++;
										}
							...
							if (H_MargeData[kokyakuno][margeCounter][0] != H_DummyTel[kokyakuno][0]) //$detailNo が初期値で
								...
								//既に取り込み処理をしている場合は $detailNo を続き番号に付け替える
								{
									...
									A_teldetailsOutputBuff.push(A_pactid[pactCounter] + "\t" + H_MargeData[kokyakuno][margeCounter][0] + "\t" + H_MargeData[kokyakuno][margeCounter][2] + "\t" + H_Code[H_MargeData[kokyakuno][margeCounter][2]] + "\t" + H_MargeData[kokyakuno][margeCounter][3] + "\t" + H_MargeData[kokyakuno][margeCounter][5] + "\t" + detailNo + "\t" + now + "\t" + G_CARRIER_KDDI_KOTEI + "\t" + H_MargeData[kokyakuno][margeCounter][4] + "\n");
								}
							...
							for (var code of Object.values(Object.keys(H_DummyCharge))) //消費税の場合
							{
								if (TAX == code) {
									if (0 != H_DummyCharge[code] - sumTax) //$sumChargePact = $sumChargePact + ($H_DummyCharge[$code] - $sumTax);		// 会社単位合計
										{
											A_teldetailsOutputBuff.push(A_pactid[pactCounter] + "\t" + H_DummyTel[kokyakuno][0] + "\t" + code + "\t" + H_Code[code] + "\t" + (H_DummyCharge[code] - sumTax) + "\t\\N\t" + detailNo + "\t" + now + "\t" + G_CARRIER_KDDI_KOTEI + "\t\n");
											detailNo++;
										}
									...
									{
										A_teldetailsOutputBuff.push(A_pactid[pactCounter] + "\t" + H_DummyTel[kokyakuno][0] + "\t" + code + "\t" + H_Code[code] + "\t" + H_DummyCharge[code] + "\t\\N\t" + detailNo + "\t" + now + "\t" + G_CARRIER_KDDI_KOTEI + "\t\n");
										detailNo++;
...
if (filesize(teldetailsFile) != 0) //tel_details_X_tb へのインポートが失敗した場合
	{
		var A_teldetails_col = ["pactid", "telno", "code", "codename", "charge", "taxkubun", "detailno", "recdate", "carid", "tdcomment"];
