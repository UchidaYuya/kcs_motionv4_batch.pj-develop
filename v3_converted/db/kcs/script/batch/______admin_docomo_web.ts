...
class ProcessAdminDocomoWeb extends ProcessBase {
	ProcessAdminDocomoWeb(procname, logpath, opentime) //サイト構成変更関連の値を初期化する
	//5分待機する
	//クランプIDは30個まで許容する
	//クランプIDあたり3回だけ許容する
	//復帰はDEBUG扱い
	...
	//必要ならプロキシの設定を読み出す
	{
		...
		var msg = "\u30D7\u30ED\u30AD\u30B7\u4F7F\u7528\u305B\u305A";
		if (this.m_proxy_host.length && this.m_proxy_port.length) msg = "\u30D7\u30ED\u30AD\u30B7\u306F" + this.m_proxy_host + ":" + this.m_proxy_port;
	...
	//日次ダウンロードをおこなうならtrue
	{
		...
		var is_admin = false;
		var is_daily = false;
		status = this.findId(O_db, is_try_admin, id, password, is_hotline, is_admin, is_daily, A_info);
		...
		//サイト構成変更を検出していないか上限に達したので、ループを抜ける
		{
			...
			try {
				status = this.doDownload(O_db, id, password, is_hotline, A_info, is_login, is_break, is_fail, is_change, is_unfinished, is_daily, A_daily, is_lock, is_admin);
			} catch (e) {
			...
			this.lockWeb(O_db, "lock_admin_docomo_web" + this.m_index);
			this.beginDB();
			var status2 = this.storeFile(O_db, is_try_admin, id, password, A_info, is_login, is_break, is_change, is_unfinished, is_admin, is_fail, is_daily, A_daily, is_lock);
			this.endDB(status2);
			this.unlockWeb(O_db);

			if (is_fail) //ログインに失敗した
				//何もせずに、ループを抜ける
				{} else if (is_unfinished) //申込が不十分
				//何もせずに、ループを抜ける
	...
	findId(O_db, is_try_admin, id, password, is_hotline, is_admin, is_daily, A_info) //clampweb_index_tbから、is_tryとis_runnningがfalseのidを一つ取り出す
	...
	//is_runningをtrueにする
	{
		...
		if (is_try_admin) sql += " and coalesce(is_admin,false)=false";
	...
	doDownload(O_db, id, password, is_hotline, A_info, is_login, is_break, is_fail, is_change, is_unfinished, is_daily, A_daily, is_lock, is_admin) //ダウンロードすべきファイルがなければ正常終了
	//最終ログイン日時を記録する
	...
	//is_dailyなら請求内訳と料金明細を追加
	{
		...
		var O_clamp = new download_docomo_type(this.m_listener, this.m_proxy_host, this.m_proxy_port);
		var login_status = O_clamp.login(id, password);
		...
		for (var cnt = 0; cnt < 3; ++cnt) //情報料ならダウンロードの準備をする
		{
			...
			var is_info = 1 == cnt;
			var is_bill = 2 == cnt;
			if (is_info) O_clamp.prepare_info();
		...
		if (is_daily) //wayが0なら請求内訳,1なら料金明細
			{
				for (var way = 0; way < 2; ++way) //ダウンロード可能な年月を取り出す
				{
					...
					var A_A_ym = Array();
					way ? O_clamp.prepare_daily(A_A_ym) : O_clamp.prepare_p(A_A_ym);
	...
	recover(O_db, id, password, is_hotline, is_fail, login_status_orig) {
		...
		if (1 == this.m_check_admin) {
			...
			//検査用アカウントでログイン
			{
				var sql = "select index_tb.clampid,index_tb.clamppassword";
				sql += " from clampweb_index_tb as index_tb";
				sql += " where coalesce(index_tb.is_admin,false)=true";
				sql += " and length(coalesce(index_tb.clampid,''))>0";
				sql += " and length(coalesce(index_tb.clamppassword,''))>0";
				...
				if (!login_fault) {
					try {
						var O_clamp = new download_docomo_type(this.m_listener, this.m_proxy_host, this.m_proxy_port);
						var login_status = O_clamp.login(admin_id, admin_password);
						delete O_clamp;
					} catch (e) {
						login_status = 4;
					}
	...
	storeFile(O_db, is_try_admin, id, password, A_info, is_login, is_break, is_change, is_unfinished, is_admin, is_fail, is_daily, A_daily, is_lock) //ダウンロードが必要な環境・年月・ファイル種別を取得
	//必要なものだけを格納
	...
	//管理用を試行したらフラグを立てる
	{
		...
		if (is_break || is_change || is_unfinished) //ログインが割り込まれたか、サイト構成変更されたので、
			...
			//ログインが割り込まれた場合は警告を出す
			{
				...
				if (is_break || is_unfinished) this.putError(G_SCRIPT_WARNING, errmsg);
			} else {
			result = O_db.getAll(sql);
		}
				...
				for (var type in A_type) {
					...
					for (var fname of Object.values(A_fname)) {
						if (file_exists(fname)) {
							...
							this.putError(is_change || is_break || is_unfinished ? G_SCRIPT_BATCH : G_SCRIPT_WARNING, "\u524A\u9664\u3059\u308B\u30D5\u30A1\u30A4\u30EB\u304C\u898B\u3064\u304B\u3089\u306A\u3044(\u51E6\u7406\u7D9A\u884C)" + fname);
						}
					}
				}
			}
		...
		if (is_daily && A_daily.length) //日次ファイルを保存する
			...
			//日次のgzファイルを削除する
			{
				...
				if (is_break || is_change || is_unfinished) //ログインが割り込まれたかサイト構成変更を検出したので保存しない
					{
						var A_pact = Array();
					} else {
