...
//２重起動ロック解除
//終了
//注）請求データ取込の後同じファイルを使って利用明細データを取込むのでファイルの移動はせずに終了
...
if (pactid == "all") ///kcs/data/yyyymm/ETC以下のディレクトリを開く
	//処理する契約ＩＤを取得する
	{
		...
		{
			if (is_dir(JCB_DIR + "/" + pactName) && pactName != "." && pactName != "..") {
...
//会社単位に終了ログを出力
{
	...
	if (target == "n") {
		...
		if (DEBUG_FLG) logging("INFO: \u6700\u65B0\u306E\u767B\u9332ETC\u30AB\u30FC\u30C9\u306E\u30EA\u30B9\u30C8\u3092\u53D6\u5F97 " + CARD_now_result.length + "\u4EF6 select cardno from card_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " ;");
	...
	//ファイル１行ずつ処理するwhile閉じ
	{
		...
		if (chkAsp(dbh.escape(A_pactid[cnt])) == true) //ASP使用料が数字以外で返って来た時
			{
				...
				var asp_charge = dbh.getOne("select charge from card_asp_charge_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and cardcoid = " + JCB_COID + " ;", true);
		...
		//ASP利用料がONの場合
		{
			...
			if (preg_match("/^(\\d){0,}$/", A_line[0]) == false) {
				if (DEBUG_FLG) logging("INFO:\uFF11\u884C\u76EE\u306F\u98DB\u3070\u3059 " + A_line[0]);
				continue;
			}
			...
			if (-1 !== A_prcardno.indexOf(A_line[0]) == false) {
				if (DEBUG_FLG) logging("ERROR: \u30D5\u30A1\u30A4\u30EB\u306B\u8A18\u8F09\u3055\u308C\u3066\u3044\u308B\u6CD5\u4EBA\u756A\u53F7(" + A_line[0] + ")\u3068\u767B\u9332\u3055\u308C\u305F\u6CD5\u4EBA\u756A\u53F7(" + A_prcardno.join(",") + ")\u304C\u7570\u306A\u308A\u307E\u3059 " + A_billFile[fcnt] + "(" + A_line[1] + "\u306E\u884C)");
			...
			var trg_post = dbh.getOne("select postid from card_bill_master_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and card_master_no = '" + A_line[0] + "';", true);

			if (trg_post == "") {
				if (DEBUG_FLG) logging("ERROR: \u672A\u767B\u9332\u30AB\u30FC\u30C9\u306E\u767B\u9332\u5148\u90E8\u7F72\u304C\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093" + "select postidparent from post_relation_tb where pactid = " + dbh.escape(A_pactid[cnt]) + " and level = 0;");
			...
			if (preg_match("/" + A_pactid[cnt] + "\t" + A_line[4] + "\t" + JCB_UTICODE + "\t\u901A\u884C\u6599\u91D1\t/", write_buf) == false) {
				...
				viewcnt++;
				write_buf += A_pactid[cnt] + "\t" + A_line[4] + "\t" + JCB_SEIKYU_UTICODE + "\t\u8ACB\u6C42\u91D1\u984D\t" + A_line[16].replace(/(,|")/g, "") + "\t\t" + viewcnt + "\t" + nowtime + "\t" + JCB_COID + "\t" + A_line[0] + "\n";
			} else //通行料金、請求金額の置き換え
				//通行料金加算
				//通行料置き換え
				//請求金額置き換え
				{
					preg_match_all("/" + A_pactid[cnt] + "\t" + A_line[4] + "\t" + JCB_UTICODE + "\t.*\n/", write_buf, A_match);
			...
			if (aspFlg == true && preg_match("/" + A_pactid[cnt] + "\t" + A_line[4] + "\t" + G_CODE_ASPX + "\tASP\u4F7F\u7528\u6599/", write_buf) == false) //合計行のために１つ表示順を空ける
				{
					write_buf += A_pactid[cnt] + "\t" + A_line[4] + "\t" + G_CODE_ASPX + "\tASP\u4F7F\u7528\u6599\t" + asp_charge + "\t\t100\t" + nowtime + "\t" + JCB_COID + "\t" + A_line[0] + "\n";
				}
...
if (mode == "o") //対象pactidを１つの文字列に変換
	//CARD_DETAIL_XX_TBの削除
	{
		...
		if (DEBUG_FLG) logging("INFO: " + carddetailX_tb + "\u306E\u65E2\u5B58\u30C7\u30FC\u30BF\u306E\u524A\u9664\u3092\u884C\u3044\u307E\u3057\u305F " + "delete from " + carddetailX_tb + " where pactid IN (" + pactin + ") and cardcoid in (" + JCB_COID + ");");
		...
		if (rtn != 0) //ロールバック
			{
				...
				logh.putError(G_SCRIPT_ERROR, SCRIPT_FILENAME + " JCB\u6CD5\u4EBAETC\u30AB\u30FC\u30C9\u8ACB\u6C42\u30C7\u30FC\u30BF\u53D6\u8FBC\u51E6\u7406 " + carddetailX_tb + "\u306E\u30C7\u30FC\u30BF\u30A4\u30F3\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F " + rtn.userinfo);
				dbh.rollback();
				throw die(1);
			} else {
...
function checkInfoFormat(A_line) //1,法人番号
{
	var errcnt = 0;

	if (preg_match("/^(\\d){10}$/", A_line[0]) == false) {
		if (DEBUG_FLG) logging("\u6CD5\u4EBA\u756A\u53F7\u306E\u30D5\u30A9\u30FC\u30DE\u30C3\u30C8\u304C\u9055\u3044\u307E\u3059 " + A_line[0]);
		errcnt++;
	}
