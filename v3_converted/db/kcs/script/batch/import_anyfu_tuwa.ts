...
function doMeisaiRecord(record, pactid, A_commFileData, sum) //コード表
//読込明細件数
//-- DEBUG -- * 表示してみる
...
//明細件数カウント.
{
	...
	var date = record[6 - 1].substr(0, 4) + "-" + record[6 - 1].substr(4, 2) + "-" + record[6 - 1].substr(6, 2) + " " + record[7 - 1].substr(0, 2) + ":" + record[7 - 1].substr(2, 2) + ":" + record[7 - 1].substr(4, 2);
	var totelno = record[9 - 1].trim();
	var toplace = record[8 - 1].trim();
	var tuwa_min = record[11 - 1];
