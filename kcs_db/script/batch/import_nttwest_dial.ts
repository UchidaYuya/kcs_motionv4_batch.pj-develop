	...
	// if (ereg("^-y=", _SERVER.argv[cnt]) == true) //請求年月文字列チェック
		{
			...
			// if (ereg("^[0-9]{6}$", billdate) == false) {
			...
			} else //年月チェック
				{
					...
					var month = parseInt(billdate.substring(4, 2));
...
//ここで合計金額チェックする.
{
	...
	var telno = record[1].trimEnd() + record[2].tirmEnd();
...
//最後の電話についてASP使用料を表示する.
{
	...
	//carid キャリアID
	{
		...
		fs.writeFileSync(fp_teldetails, pactid + "\t" + telno + "\t" + data[1] + "\t" + ut_name + "\t" + data[2] + "\t" + H_Zei_CODE[data[3]] + "\t" + detailNo + "\t" + nowtime + "\t" + NTTWEST_CARRIER_ID + "\t" + data[4] + "\n");// 2022cvt_006
	}
