...
// 2022cvt_015
			var tmp_dh = fs.readdirSync(dataDirPact);
			// var tmp_dh = openDir(dataDirPact);// 2022cvt_004

			for (var data_filename of tmp_dh) {
			// while (data_filename = fs.readdir(tmp_dh)) {// 2022cvt_005
// 2022cvt_019
				if (data_filename.match("/^OTMEISAI\\.txt$/i"))
				// if (preg_match("/^OTMEISAI\\.txt$/i", data_filename) == true) //OTMEISAIを配列に入れる
					{
						A_billFile.push(dataDirPact + "/" + data_filename);
						if (DEBUG_FLG) {
							logging("INFO: 対象請求データファイル名 " + dataDirPact + "/" + data_filename);
						}
						break;
					}
			}
...
// 2022cvt_019
				} else if (A_billFile[fcnt].match("/\\/Shutokou\\//"))
...
// 2022cvt_019
				} else if (A_billFile[fcnt].match("/\\/Hanshin\\//"))
...
// 2022cvt_019
				} else if (A_billFile[fcnt].match("/\\/HonShi\\//"))
...
// 2022cvt_015
		...
		if (rtn2 != 0) //ロールバック
			{
				if (DEBUG_FLG) {
					logging("ERROR: " + cardusehistoryX_tb + "のデータインポートに失敗しました " + rtn2);
				}
				logh.putError(G_SCRIPT_ERROR, SCRIPT_FILENAME + " ETCコーポレートカード請求データ取込処理 " + cardusehistoryX_tb + "のデータインポートに失敗しました " + rtn2);
				dbh.rollback();
				throw process.exit(1);// 2022cvt_009
			} else {
			if (DEBUG_FLG) {
...
// 2022cvt_019
	...
	// if (mb_strlen(A_line[8]) != 10) {
		if (DEBUG_FLG) {
			logging("出口IC名のフォーマットが違います " + A_line[8]);
		}
		errcnt++;
	}
...
// 2022cvt_019
	if (!A_line[14].match("/^(00|01|02|50)$/")) {
...
// 2022cvt_019
	if (!A_line[15].match("/^(0|1)$/")) {
	// if (preg_match("/^(0|1)$/", A_line[15]) == false) {
		if (DEBUG_FLG) {
			logging("区分１のフォーマットが違います " + A_line[15]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[16].match("/^(0|1)$/")) {
	// if (preg_match("/^(0|1)$/", A_line[16]) == false) {
		if (DEBUG_FLG) {
			logging("区分２のフォーマットが違います " + A_line[16]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[17].match("/^(0|1)$/")) {
	// if (preg_match("/^(0|1)$/", A_line[17]) == false) {
		if (DEBUG_FLG) {
			logging("区分３のフォーマットが違います " + A_line[17]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[18].match("/^(0|1)$/")) {
	// if (preg_match("/^(0|1)$/", A_line[18]) == false) {
		if (DEBUG_FLG) {
			logging("区分４のフォーマットが違います " + A_line[18]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[19].match("/^(0|1|2)$/")) {
	// if (preg_match("/^(0|1|2)$/", A_line[19]) == false) {
		if (DEBUG_FLG) {
			logging("区分５のフォーマットが違います " + A_line[19]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[20].match("/^(0|1)$/")) {
	// if (preg_match("/^(0|1)$/", A_line[20]) == false) {
		if (DEBUG_FLG) {
			logging("区分６のフォーマットが違います " + A_line[20]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[21].match("/^(0|2|3|4|5|6|7|8)$/")) {
	// if (preg_match("/^(0|2|3|4|5|6|7|8)$/", A_line[21]) == false) {
		if (DEBUG_FLG) {
			logging("区分７のフォーマットが違います " + A_line[21]);
		}
		errcnt++;
	}
