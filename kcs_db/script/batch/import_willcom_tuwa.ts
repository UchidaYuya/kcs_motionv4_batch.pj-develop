...
class WillcomProcessBase {
	...
	A_types: number[];
	Kanri_POS: number[] = [];
	HEAD_LINE_LENGTH: number = 0;
	BODY_LINE_LENGTH: number = 0;
	WILLCOM_PAT: string = "";
	TEL_TYPE: string = "";
	Meisai_POS: number[] = [];
	//doTelRecordPtr: string = "";
	doTelRecordPtr: ( a: any,  b: string, c: { [key: string]: number }, d: string) =>any = function(line: any, pactid: string, sum: { [key: string]: number }, fileName: string){};
	doEachLinePtr: ( a: string,  b: any, c: any, d: { [key: string]: number }, e: string, f: { [key: string]: number }) =>any = function(dataKind: string, line: any, pactid: any, TotalUp: { [key: string]: number }, fileName: string, sum: { [key: string]: number }){};
	pactid_in: string = "";
	billdate: string = "";
	mode: any;
	backup: any;
	...
	async MainProcess(argv: string | any[]) //開始メッセージ
	...
	//ここまでに成功したファイルが無ければ終了する.
	{
			...
			// if (ereg("^-y=", argv[cnt]) == true) //請求年月文字列チェック
				{
					...
					// if (ereg("^[0-9]{6}$", this.billdate) == false) {
					...
					} else //年月チェック
						{
							...
							year = parseInt(this.billdate.substring(0, 4));

							month = parseInt(this.billdate.substring(4, 2));
		...
		if (this.pactid_in == "all") //処理する契約ＩＤを取得する
			//契約ＩＤが指定されている場合
			{
				...
				{
					if (fs.existsSync(this.dataDir + "/" + fileName) && fileName != "." && fileName != "..") {
...
// 					clearstatcache();
		...
		var fp_totel;
		try {
			fs.openSync(this.totelmasterFile, "w");
		} catch (e) {
		...
		//正常処理が終わった pactid のみ書き出し処理
		{
			...
			for (var ctype of this.A_types) //処理するファイル種別ごとのパラメータ設定
			//処理する請求データファイル名配列
			//通話明細データファイル名を取得する
			//通話明細データファイルがなかった場合
					...
					// if (is_file(dataDirPact + "/" + fileName) == true) //ファイル名の先頭文字が適合するものだけ
						{
							...
							if (this.WILLCOM_PAT.match(fileName)) {
...
// 					clearstatcache();
	...
	doEachLine(dataKind: string, line: any, pactid: any, TotalUp: { [key: string]: number }, fileName: string, sum: { [key: string]: number }) //10:通話単位
	{
	...
	doEachLinePacket(dataKind: string, line: any, pactid: any, TotalUp: { [key: string]: number }, fileName: string, sum: { [key: string]: number }) //10:通話単位
	{
		if (dataKind == "10") //動的に関数を呼び出す
			//if( $this->doTelRecord_C0($line, $pactid, &$sum) == 1 ){
			{
	...
	doEachLineInfo(dataKind: string, line: any, pactid: any, TotalUp: { [key: string]: number }, fileName: string, sum: { [key: string]: number }) //10:通話単位
	{
		if (dataKind == "10") //動的に関数を呼び出す
			//if( $this->doTelRecord_C0($line, $pactid, &$sum) == 1 ){
			{
	...
	checkKanriRecord(line: string, fileName: string, pactid: string, A_codes: Array<any>) //１行の長さチェック
	//データ種類チェック
		...
		if (errFlag == true) //請求先番号が存在しない場合のメール送信予約追加20101202morihara
			{
				...
				var sql = "insert into clamp_error_tb" + "(pactid,carid,error_type,message,is_send,recdate,fixdate)" + "values" + "(" + pactid + "," + WILLCOM_CARRIER_ID + ",'prtelno'" + ",'ソフトバンクの請求先番号が登録されている親番号と異なっています'" + ",false" + ",'" + new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '').replace("-", "/") + "'" + ",'" + new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '').replace("-", "/") + "'" + ")" + ";";
				this.dbh.query(sql);
				return 1;
			}
	...
	doTelRecord_C0(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C2(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doPacketRecord(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C3(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C4(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C5(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C6(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTelRecord_C7(line: any, pactid: string, sum: { [key: string]: number }, fileName: string) //分割してみる.
	//DEBUG * 表示してみる
	...
	doTotalRecord(line: any, TotalUp: { [key: string]: number }) //分割してみる.
	//DEBUG * 表示してみる
	//print "==== doTotalRecord ================================\n";
	//foreach( $record as $item ){ print "\"" .  $item . "\"\n"; }
	...
	splitFix(str: string, A_pos: string | any[]) //１個目の要素
	//中間の要素
	//最後の要素
	{
		...
		Encoding.convert(A_ret, { from: "SJIS", to: "UNICODE", type: "string" });
...
// 	doCopyExp(sql: string, filename: string, db: ScriptDB) //一度にFETCHする行数
// 	//ファイルを開く
// 	{
//
...
// 			clearstatcache();
	...
	usage(argv: string | any[], comment: string) //ロック解除
	{
		if (comment == "") {
			comment = "パラメータが不正です";
		}
