...
if (pactid_in == "all") //処理する契約ＩＤを取得する
	//契約ＩＤが指定されている場合
	{
		...
		// while (fileName = fs.readdirSync(dirh)) //カレントと親ディレクトリを除いたディレクトリ名（＝契約ＩＤ）を配列へ格納する
		{
...
//正常処理が終わった pactid のみ書き出し処理
{
	...
	// while (fileName = fs.readdir(dirh)) {
		...
		// if (is_file(dataDirPact + "/" + fileName) == true) //ファイル名の先頭文字が適合するものだけ
			{
				console.log(fileName.match(NTTEAST_PAT))
				if (fileName.match(NTTEAST_PAT)) {
				// if (preg_match(NTTEAST_PAT, fileName)) {
					A_billFile.push(fileName);
				} else {
					console.log(G_SCRIPT_INFO, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + LOG_DELIM + billdate + LOG_DELIM + dataDirPact + "/" + fileName + "は処理されませんでした.");
...
// 		clearstatcache();
	...
	if (rootPostid == "") //次のPactの処理にスキップする.
		{
			console.log(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid_in + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + "post_relation_tbからRoot部署の取得にに失敗.");
	...
	if (rootPostidX == "") //次のPactの処理にスキップする.
		{
			console.log(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid_in + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + postrelX_tb + "からRoot部署の取得にに失敗.");
	...
	//追加する電話数
	{
		...
		logh.putError(G_SCRIPT_INFO, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + dataDirPact + "/" + fileName + LOG_DELIM + "データ読込処理完了:電話件数=" + ReadTelCnt + ":明細件数=" + AddTelCnt + ":追加電話件数(tel_tb)=" + telX_tb + ")=" + AddTelXCnt);
	}
...
//最後の電話についてASP使用料を表示する.
{
	...
	//carid キャリアID
	{
		...
		fs.writeFileSync(fp_teldetails, pactid + "\t" + telno + "\t" + ut_code + "\t" + ut_name + "\t" + data[2] + "\t" + H_Zei_CODE[data[3]] + "\t" + detailNo + "\t" + nowtime + "\t" + NTTEAST_CARRIER_ID + "\t" + data[4] + "\n");// 2022cvt_006
	}
