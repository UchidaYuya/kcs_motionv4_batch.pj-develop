...
import { G_MAIL_SUBJECT, G_MAIL_TYPE } from '../../conf/batch_setting';
import { G_MAILINGLIST, G_MAIL_FROM, G_MAIL_TO, G_SMTP_HOST, G_SMTP_PORT } from '../../db_define/define';
...
export const G_PHP_TLS12 = "/usr/local/bin/php_tls12";

const Iconv  = require('iconv').Iconv;
export default class ScriptLogMail extends ScriptLogBase {
	m_tls12: boolean | undefined;
	m_A_body: string[];
	...
	send() //配列->文字列
	{
		...
		for (var line of this.m_A_body) {
			if (-1 !== line.indexOf("正常終了")) continue;
			is_fail = true;
			break;
		}
		...
		const title = is_fail ? "バッチ処理でエラー発生": "バッチ処理正常終了";

		const iconv3 = new Iconv("UTF-8", 'JIS');
		body = iconv3.convert(title + "(" + G_MAIL_TYPE + ")\n\n") + body;
		// let O_mail = Mail.factory("smtp", {
		// 	host: G_SMTP_HOST,
		// 	port: G_SMTP_PORT
		// });　一旦コメンアウト
		let O_mail
