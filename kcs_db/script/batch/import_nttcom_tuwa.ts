...
export const G_WORLD_SERVICE = "30";

//ＮＴＴ西日本prime
//一行(固定長)を分割して処理する
export default class import_ntt_com extends import_ntt_tuwa_base {
	m_first_flag!: boolean;
	m_procname_j!: string;
	m_target_pactid!: string;
	m_db!: ScriptDB;
	m_year: any;
	m_compnam: any;
	m_compname: string | Promise<any> | undefined;
	m_targetFile!: string;
	m_targetLine: any;
	m_warningCount!: number;
	m_h_wld_area: any;
	m_h_comm_area: any;
	m_h_tuwa_kind: any;
	m_h_daynight_wld: any;
	m_h_daynight: any;
	m_insertCount: any;
	m_warningCountGross: any;
	m_pactid: string = "";
	m_srcpath: string = "";
	m_fileCount: any;
	m_insertCountGross: any;
	getTableNo: any;
...
// 2022cvt_015
				var serviceCode = line.substring(28, 2);
...
// 						echo(`警告：課金先電話番号(${bill_telno})が不正です。登録されている課金先電話番号と合致しません。file:${this.m_targetFile} (` + this.m_targetLine + "\u884C\u76EE) \n");// 2022cvt_010
						// GLOBALS.G_COMMON_LOG.putError(SCRIPTLOGBASE.G_SCRIPT_WARNING, this.m_procname_j + " " + this.m_target_pactid + " " + this.m_compname + " " + yyyymm + " " + this.m_targetFile + ` 警告：課金先電話番号(${bill_telno})が不正です。登録されている課金先電話番号と合致しません。file:${this.m_targetFile} (` + this.m_targetLine + "行目)", 4);
...
// 2022cvt_015
					...
					var startd;
// 2022cvt_021
					startd.datetime_str = sprintf("%04d/%02d/%02d %02d:%02d:%02d", YYYY, MM, DD, H, M, S);
					startdatetime = Date.parse(startdatetime_str);
				}
// 2022cvt_015
				var to_Telno = line.substring(56 - 1, 13);

				if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
// 2022cvt_015
					var ken_code = line.substring(70, 2);
// 2022cvt_015
					var ken_name = this.m_h_wld_area[ken_code];
				} else {
					ken_code = line.substring(69 - 1, 2);
					ken_name = this.m_h_comm_area[ken_code];
				}
...
// 2022cvt_015
				...
				//0.5秒単位＝7桁
				{
					if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
// 2022cvt_015
						var tuwa_time = line.substring(72, 8);
					} else {
						tuwa_time = line.substring(71 - 1, 8);
					}
...
// 2022cvt_015
				var tuwa_money_str = "";
				{
					if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
// 2022cvt_015
						var tuwa_money = line.substring(80, 10);
					} else {
						tuwa_money = line.substring(79 - 1, 10);
					}
...
// 2022cvt_015
				...
				if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
// 2022cvt_015
					var tuwa_kind_code = line.substring(103, 4).trim();
// 2022cvt_015
					var tuwa_kind_name = this.m_h_tuwa_kind[tuwa_kind_code];
				} else {
					tuwa_kind_code = line.substring(111 - 1, 3);
				}

				tuwa_kind_name = this.m_h_tuwa_kind[tuwa_kind_code];

				if (undefined !== serviceCode && G_WORLD_SERVICE == serviceCode) {
// 2022cvt_015
					var hiruyoru_code = line.substring(107, 3);
// 2022cvt_015
					var hiruyoru_name = this.m_h_daynight_wld[hiruyoru_code];
				} else {
					hiruyoru_code = line.substring(114 - 1, 3);
					hiruyoru_name = this.m_h_daynight[hiruyoru_code];
				}

				if (!(undefined !== ken_name) || "" == ken_name) {
