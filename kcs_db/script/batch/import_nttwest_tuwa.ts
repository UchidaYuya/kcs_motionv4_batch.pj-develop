...
import { sprintf, G_LOG } from "../../db_define/define";
import {
	G_SCRIPT_WARNING,
	G_SCRIPT_ERROR,
	G_SCRIPT_ALL,
	...
	ScriptLogFile,
	ScriptLogAdaptor
} from "../../script/batch/lib/script_log";
import * as KCS_DIR from "../../class/MtSetting";

export const G_OPENTIME = "import_ntt_tuwa_base"
export const G_PROCNAME_J_IMPORT = "通話詳細インポートプロセス[NTT西日本]";
export const G_PROCNAME_IMPORT = "import_ntt_west";
export const G_PROCNAME = "import_ntt_west";
export const G_COMPANY_ID = "NTT_west/tuwa";
export const G_CARRIER_ID = "8";
...
//ＮＴＴ西日本custom
//一行(固定長)を分割して処理する
export default class import_ntt_west extends import_ntt_tuwa_base {
	async doDivisionLine(line: string, ins_comm: any, a_bill_prtel: any) //バグ修正 戻り値がデータにより帰らない 2004.12.07 by suehiro
	//Prime / Custom 別の処理
	//頭文字取得
...
// 2022cvt_015
		var res = false;
// 2022cvt_015
		var caption = this.m_file_name.substring(0, 1);
		var yyyymm = this.m_year + this.m_month;

		if (caption == "R") //prime
			//バグ修正 戻り値がデータにより帰らない 2004.12.07 by suehiro
			{
				res = await this.doDivisionLineForPrime(line, ins_comm, a_bill_prtel);
			} else if (caption == "M") //custom
			//バグ修正 戻り値がデータにより帰らない 2004.12.07 by suehiro
			{
				res = await this.doDivisionLineForCustom(line, ins_comm, a_bill_prtel);
			} else //想定外ファイル
			//バグ修正 戻り値がデータにより帰らない 2004.12.07 by suehiro
			{
// 				echo("\u8B66\u544A\uFF1A \u51E6\u7406\u5BFE\u8C61\u306E\u30D5\u30A1\u30A4\u30EB\u540D\u79F0(" + this.m_file_name + ")\u304C\u4E0D\u6B63\u3067\u3059\u3002M\u304BR\u3067\u59CB\u307E\u308B\u30D5\u30A1\u30A4\u30EB\u540D\u306B\u3057\u3066\u304F\u3060\u3055\u3044\u3002\n");// 2022cvt_010
				global.G_COMMON_LOG.putError(G_SCRIPT_WARNING, this.m_procname_j + " " + this.m_target_pactid + " " + this.m_compname + " " + yyyymm + " " + this.m_targetFile + "警告： 処理対象のファイル名称(" + this.m_file_name + ")が不正です。MかRで始まるファイル名にしてください。", 4);
				res = false;
			}
	...
	async doDivisionLineForPrime(line: string, ins_comm: {
			insert: (arg0: {
				pactid: string; telno: any;
				// 2022cvt_016
				type: string; date: any; totelno: any; toplace: any; time: string; charge: string; callseg: any; callsegname: any; chargeseg: any; carid: string;
			}) => void;
		}, a_bill_prtel: string | any[]) //レコード種別:G_RECODE_IDのみ
	{
// 2022cvt_015
		...
		if (recode_id == G_RECODE_ID) //課金先電話番号
			...
			{
// 2022cvt_015
				var bill_telno = line.substring(2 - 1, 10);
...
// 						echo("\u8AB2\u91D1\u5148\u96FB\u8A71\u756A\u53F7(" + bill_telno + ")\u304C\u8907\u6570\u5B58\u5728\u3057\u307E\u3059\u3002\n");// 2022cvt_010
						global.G_COMMON_LOG.putError(G_SCRIPT_WARNING, this.m_procname_j + " " + this.m_target_pactid + " " + this.m_compname + " " + yyyymm + " " + this.m_targetFile + "警告： 課金先電話番号(" + bill_telno + ")が複数存在します。", 4);
						return false;
					}
				}
...
// 								echo(`警告：データの請求月が処理対象月と異なります。file:${this.m_targetFile} (` + this.m_targetLine + "\u884C\u76EE) \n");// 2022cvt_010
								global.G_COMMON_LOG.putError(G_SCRIPT_ERROR, this.m_procname_j + " " + this.m_target_pactid + " " + this.m_compname + " " + yyyymm + " " + this.m_targetFile + ` 警告:データの請求月が処理対象月と異なります。file:${this.m_targetFile} (` + this.m_targetLine + "行目)", 3);
								this.m_warningCount++;
								return false;
							}
					}
...
// 2022cvt_021
					startdatetime_str = sprintf("%04d/%02d/%02d %02d:%02d:%02d", YYYY.toString(), MM, DD, H, M, S);
...
// 				}
	...
	async doDivisionLineForCustom(line: string, ins_comm: {
			insert: (arg0: {
				pactid: string; telno: any;
				// 2022cvt_016
				type: string; date: any; totelno: any; toplace: any; time: string; charge: string; callseg: any; callsegname: any; chargeseg: any; carid: string;
