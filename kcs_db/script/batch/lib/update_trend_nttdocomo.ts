...
//プレフィックス("tel_tb."を想定)
//返値：ANDで始まるSQL文の一部
export default class updateTrendNttDocomo extends ScriptDBAdaptor {
	m_inserter_trend: any;
	m_inserter_toptel: any;
	key: any;
	H_Kamoku_Key: { 140: number; 141: number; 142: number; 143: number; 144: number; 145: number; 146: number; } | any;
	DocomoTuwaTypes: any;
	DomesticTuwaTypes: any;
	// constructor() {
	// 	super(...arguments);
	...
	constructor(listener: ScriptLogBase | any, db: string, table_no: string, inserterTrend: any, inserterToptel: any) //シミュレーション科目 => key の対応表
	{
...
// 2022cvt_015
	...
	executeTimezone(pactid: any, carid: string, table_no: string) //平日の08:00～19:00なら平日昼(0)
	//tdow=1は日曜日
	//それ以外なら(1)
	//時
	//曜日
...
// 2022cvt_015
	...
	executeIsmobile(pactid: any, carid: string, table_no: string) //080か090で始まっていたらキーは1で、それ以外はゼロ
	//通話タイプ
	{
		var H_line: { value: number; };
...
// 2022cvt_015
	...
	executePacketCnt(pactid: string, carid: string, table_no: string) //// まず通話明細を取得する	-- 2007/06/15
...
// 2022cvt_016
	...
	{
		var line: { telno: string; totelno: any; sum_byte: number; value: number; simkamoku: any; };
...
// 2022cvt_015
			...
			if (line.totelno.toMatch(FULL_BROWSER_PATTERN)) //'iﾓｰﾄﾞ(ﾌﾙﾌﾞﾗｳｻﾞ)'と入ってくる
				{
					sum_browse += line.sum_byte;
				} else if (line.totelno.toMatch(I_MODE_PATTERN)) //'iﾓｰﾄﾞ'と入ってくる
				{
...
// 2022cvt_015
			...
			var p_mode: number, p_browse: number, p_other: number;
...
// 2022cvt_015
	...
	writePacketCnt(H_meisai: any[], H_result: any[], prev_tel: any) //明細から得られた値で上書きする
	//書き込み処理
	{
...
// 2022cvt_015
		for ( key in H_result_key) //print_r( $inline );	// DEBUG
		{
// 2022cvt_015
...
// 2022cvt_015
	...
	executePacketRatio(pactid: any, carid: string, table_no: string) //通話明細から比率を求める
	//byteの有無で通話/通信を判断する
	//電話毎の結果
	{
		var line: { telno: string; totelno: any; };
...
// 2022cvt_015
		...
		while (line = result.fetchRow(DB_FETCHMODE_ASSOC)) {
			if (cur_telno.localeCompare(line.telno)) //電話番号が切り替わった
				{
					if (H_result.length) {
	...
	executePacketRatioTelno(pactid: any, carid: any, telno: string, H_result: {} | any[]) //総バイト数を求め、それがゼロなら何もせずに戻る
	//各種別のパケット比率を記録する
	//メール
	{
// 2022cvt_015
...
// 2022cvt_015
		for (var line of Object.values(H_result)) {
			if (line.totelno.toMatch(I_MODE_PATTERN)) //かけ先がiモードなら、種別をそのまま集計する
				{
...
// 2022cvt_015
	...
	getDaytimeRatio(pactid: string, carid: string, table_no: string) //2006/12/19 by T.Naka
	//最後の電話
	{
		var line: { telno: string; key: number; value: number; };
		...
		sql += " order by telno";
// 2022cvt_015
		var H_result:{[key:string]:number} = {};
// 2022cvt_015
		var result = this.m_db.query(sql);
// 2022cvt_015
		var prev_tel : string;
...
// 2022cvt_015
		...
		while (line = result.fetchRow(DB_FETCHMODE_ASSOC)) //電話番号が変わったら
		{
			if (prev_tel !== "" && line.telno !== prev_tel) {
				if (val0 + val1 > 0) {
// 2022cvt_015
					var ratio:number = +(val0 / (val0 + val1) * 100);
...
// 2022cvt_016
	...
	{
		var line: { telno: any; code: string; key: number; value: any; charge: number; plan_tb_charge: any; nightcharge: number; plan_tb_nightcharge: any; sum_charge: number; chgunit: number; };
		var H_telno = Array();
// 2022cvt_015
...
// 2022cvt_015
	...
	//
	{
		var line: { code: string; key: number; value: number; };
		var sql = "select telno";
		sql += ",count(*) as value";
		sql += " from commhistory_" + table_no + "_tb";
		sql += " where pactid=" + this.escape(pactid);
...
// 2022cvt_015
	...
	executeToptel(pactid: any, carid: string, table_no: string) //国内通話タイプ
	//2006/12/19 by T.Naka
	{
		var line: { telno: string; recid: number; sec: number; };
