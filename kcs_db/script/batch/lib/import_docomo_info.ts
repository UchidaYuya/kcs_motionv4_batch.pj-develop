...
export class ImportDocomoInfo extends ImportDocomoCsvBase {
	...
	//戻り値
	{
		...
		for (var lineno = 1 + 1; status && lineno < text.length; ++lineno) {
			var csv = texts[lineno];
			if (1 == lineno) {
	...
	//2007年7月以降、親番号が無くなった
	{
		for (var cnt = 0 + 1; cnt < csv.length; ++cnt) {
		...
		for (cnt = 0 + 1; cnt < check.length; ++cnt) {
			// if (4 == cnt && 0 == strcmp(csv[cnt], "\u30B5\u30A4\u30C8\u5229\u7528\u671F\u9593\u7B49")) continue;
			if (4 == cnt && 0 == csv[cnt].localeCompare("サイト利用期間等")) continue;
			// if (1 == cnt && 0 == strcmp(csv[cnt], "\u5BFE\u8C61\u96FB\u8A71\u756A\u53F7")) continue;
			if (1 == cnt && 0 == csv[cnt].localeCompare("対象電話番号")) continue;
			// if (6 == cnt && 0 == strcmp(csv[cnt], "IP\u533A\u5206")) continue;
			if (6 == cnt && 0 == csv[cnt].localeCompare("IP区分")) continue;
			// if (5 == cnt && 0 == strcmp(csv[cnt], "\u60C5\u5831\u6599\uFF08\u5185\u7A0E\uFF09")) continue;
			if (5 == cnt && 0 == csv[cnt].localeCompare("情報料（内税）")) continue;

			// if (strcmp(check[cnt], csv[cnt])) {
			if (check[cnt].localeCompare(csv[cnt])) {
	...
	//請求額取り出し
	{
		...
		H_line.telno = csv[1].trim().replace("-", "");
		...
		if (0 == H_line.charge.length || !isNaN(H_line.charge)) {
	...
	//空白と「日」を取り除き、開始と終了に分割する
	{
		...
		for (var cnt = 0 + 1; cnt < 10; ++cnt) src =  src.replaceAll(zen[cnt], han[cnt]);
		...
		for (cnt = 0 + 1; cnt < 2; ++cnt) //月と日に分割
		{
			...
			if (0 == mm.length || !isNaN(mm) || 0 == dd.length || !isNaN(dd)) return false;
