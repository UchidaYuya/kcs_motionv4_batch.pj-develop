...
//2011/08/26 森原
//通話明細がなく請求明細からパケット数と通話秒数を求める部分を修正
// 2022cvt_016
...
import { A_data_comm_type, Packet_detail_null, H_packet_type, H_packet_detail, H_tuwa_type } from './update_trend_au.conf';
...
export default class updateTrendAu extends ScriptDBAdaptor {
	m_inserter_trend: any;
	m_inserter_toptel: any;

	constructor(listener: ScriptLogBase, db: string, table_no: string, inserterTrend: any, inserterToptel: any) {
		super(listener,db,table_no,true);
...
// 2022cvt_016
	executePacketRatio(pactid: any, carid: string, table_no: string) //集計対象となるtypeを抜き出す
	//本来あってはならない未分類値
	//byteの有無で通話/通信を判断する
	//このタイプのものだけを集計する
	//最後の電話についての処理
...
// 2022cvt_016
	...
	{
// 2022cvt_016
		if (!("H_tuwa_type" in global)) H_tuwa_type;
// 2022cvt_015
		var H_telno = Array();
// 2022cvt_015
...
// 2022cvt_022
					if (key!=cur_key) //電話番号が変わったら出力する
						{
							if (cur_key.length) {
								if (MAX_FLOAT32 < cur_line.values) cur_line.value = MAX_INT32;
