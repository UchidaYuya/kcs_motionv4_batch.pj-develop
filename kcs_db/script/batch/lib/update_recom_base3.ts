...
import { G_CARRIER_AU, G_CARRIER_DOCOMO, G_CIRCUIT_FOMA, G_CIRCUIT_MOVA } from "./script_common";
import TableNo, { FetchAdaptor, ScriptDB, ScriptDBAdaptor, TableInserterBase } from "./script_db";
import { G_SCRIPT_DEBUG, G_SCRIPT_ERROR, G_SCRIPT_INFO, G_SCRIPT_WARNING, ScriptLogBase } from "./script_log";
...
//平均値
//ログ出力に必要なデバッグレベル
export class UpdateRecomBase extends ScriptDBAdaptor {
	static s_H_plan;
	static s_H_packet;
	static s_A_group_planid;
	static s_A_group_packetid;
	static g_log_normal = 448;
	static g_log_result = 256;
	static g_log_talkcomm = 8;
	static g_log_ave = 4;
	static g_log_convert = 4;
	m_H_buysel_empty: { [x: number]: number; 1: number; 3: number; };
	m_O_inserter_log: TableInserterBase;
	m_assert: any;
	m_carid_after: any;
	m_A_cirid_after: any;
	m_A_cirid_packet_zero: any;
	m_A_buysel: any;
	m_penalty_limit: any;
	m_ratio_same_carrier: any;
	m_min_planid: any;
	m_min_packetid: any;
	m_arid_common: any;
	m_H_disratio: any[];
	m_H_packetid_disratio_pact: any[];
	m_A_pactid_disratio_pact: any[];
	m_log_index_code: number;
	m_log_index_key: number;
	m_log_index_value: number;
	m_log_index_memo: number;
	m_log_index_planid: number;
	m_log_index_packetid: number;
	m_H_plan: any[];
	m_H_packet: any;
	m_A_group_planid: any[];
	m_A_group_packetid: any[];
	m_H_penalty: any[];
	m_H_planid_cirid: any[];
	m_H_planid_data: any[];
	m_H_planid_buysel: any[];
	m_H_packetid_cirid: any[];
	m_H_packetid_hodai: any[];
	m_A_packet_zero_all: any[];
	m_H_packet_zero: any[];
	m_A_predata_disratio: string[];
	m_A_predata_required: string[];
	m_H_ave_trend_default: any;
	m_A_smpcirid_smart: any[];
...
// 2022cvt_015
				for (var H_line of result) H_buysel[H_line.buyselid] = H_line;
...
// 2022cvt_015
				for (var H_line of result) //メールホーダイ系ならデータ専用フラグをfalseにする
				//買い方が無ければ買い方なしとする
...
// 2022cvt_015
				for (var H_line of result) //無料パケットその他を入れる(ダミーなので常にゼロ)
...
// 2022cvt_015
		for (var cirid of this.m_A_cirid_after) this.m_H_planid_cirid[cirid] = Array();
...
// 2022cvt_015
		for (var buysel of this.m_A_buysel) this.m_H_planid_buysel[buysel] = Array();

		{
			let _tmp_4 = this.m_H_plan;

// 2022cvt_015
			for (let planid in _tmp_4) {
// 2022cvt_015
...
// 2022cvt_015
		for (var cirid of this.m_A_cirid_after) this.m_H_packetid_cirid[cirid] = Array();
...
// 2022cvt_015
	...
	getValue(H_src: {} | any[], key: string | number, def: number | boolean) {
...
// 2022cvt_015
	...
	get_buysel(is_penalty: boolean, H_tel: any, buysel_before: any, is_change_course: string, is_change_carrier: any) //派生型で実装する
	{
		is_penalty = false;
	...
	calc(is_error: boolean, A_details: {} | any[], H_tel:any, H_ave: {} | any[], A_param: {} | any[], H_disratio: {} | any[], pactid: any, monthcnt: any, cur_date: any, is_change_carrier: any) //実行パラメータを取り出す
	{
		is_error = false;
// 2022cvt_015
		var H_param;
...
// 2022cvt_015
					...
					H_log[this.m_O_inserter_log.getIndex("recdate")] = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '') ;
...
// 2022cvt_015
	...
	calc_result(A_details:any, H_tel: any, H_param:any, H_ave: any, H_result_hodai: {} | any[], penalty: number | undefined, is_change_hodai: string | number, is_change_course: string | number, is_change_carrier: any) //1件目があればtrue
	//現在のプラン・パケット
	//削減前の値を集計する
...
// 2022cvt_015
			for (var src of A_src) H_details[tgt] += H_ave.predata[src];
...
// 2022cvt_015
	...
	calc_result_item(H_details: any, H_tel: {} | any[], H_param: {} | any[], H_ave: {} | any[], curkey: string, minkey: string, H_charge: any, postfix: string, is_change_carrier: any, is_poor: number, is_result_1: any) //現在と同じ買い方
	//元のプランを取り出す
	{
// 2022cvt_015
...
// 2022cvt_015
			for (var packetid of this.m_H_packetid_disratio_pact[cirid]) {
...
// 2022cvt_015
		for (let is_change_course of A_is_change_course) {
// 2022cvt_015
...
// 2022cvt_015
				if (undefined !== this.m_H_planid_buysel[buysel]) for (var id of this.m_H_planid_buysel[buysel]) if (!(-1 !== A_id.indexOf(id))) A_id.push(id);
			}

			H_planid_course[is_change_course] = A_planid.filter((value: any) => A_id.includes(value));
		}
...
// 2022cvt_015
	...
	get_min_key_plan(minkey: string) {
		if (minkey.indexOf(",")) {
// 2022cvt_015
			var A_minkey = minkey.split(",");
			return A_minkey[0];
	...
	get_min_key_packet(minkey: string) {
		if (minkey.indexOf(",")) {
// 2022cvt_015
			var A_minkey = minkey.split(",");
			return A_minkey[1];
	...
	get_cur_plan(planid: string | number) {
	...
	calc_all_w_packet(H_result_hodai: any[], H_is_change_hodai: any, H_tel: any, H_log: {} | any[], H_ave: any, H_param: {} | any[], H_disratio: {} | any[], A_planid: {} | any[], A_packetid: {} | any[], H_planid_course: {} | any[], H_packetid_hodai: {} | any[], memo: any, is_change_carrier: any, is_data: boolean) //全プランに対してループする
...
// 2022cvt_015
					for (var planid of A_planid_course) if (-1 !== this.m_H_planid_buysel[buysel_before].indexOf(planid)) A_planid_buysel.push(planid);
				}
...
// 2022cvt_015
				var packetid: any = minkey_packet;
// 2022cvt_015
				var H_plan = this.m_H_plan[planid];
// 2022cvt_015
				var H_packet = this.m_H_packet[packetid];
...
// 2022cvt_015
			var H_comm: any = this.calc_H_comm_from_plan(H_log, H_ave, H_param, H_tgt_plan, is_transparent_packet);
		} else {
...
// 2022cvt_015
		var H_other: any = this.calc_talkcomm_other(H_log, H_ave);
...
// 2022cvt_015
		var raw_talk = this.calc_raw_talk(H_log, H_ave, H_param, H_tgt_plan, H_touch, is_transparent_plan, this.m_ratio_same_carrier);
// 2022cvt_015
		var H_comm: any = this.calc_H_comm_from_plan(H_log, H_ave, H_param, H_tgt_plan, is_transparent_plan);
// 2022cvt_015
		var limit_mode = H_comm.mode;
		if (undefined !== H_tgt_packet.limit_mode && 0 < H_tgt_packet.limit_mode && H_tgt_packet.limit_mode < limit_mode) limit_mode = H_tgt_packet.limit_mode;
		this.insert_log(H_log, "limit_mode", 0, limit_mode, memo);
...
// 2022cvt_015
		for (var H_penalty of A_penalty) {
...
// 2022cvt_015
		var end_year = new Date(cur_date).getFullYear();
// 2022cvt_015
		var end_month = new Date(cur_date).getMonth();
// 2022cvt_015
...
// 2022cvt_015
		...
		start_date = +new Date(start_date).getDate();
...
// 2022cvt_022
		if (1 == result.length && 0 == "H".localeCompare(result[0][0])) is_hotline = true;
...
// 2022cvt_015
		var sql = "";
// 2022cvt_015
		var A_src: any = [[A_telno_in, ""], [A_telno_out, "not"]];
...
// 2022cvt_015
			for (var A_line of A_result) A_new.push(A_line[0]);

			A_new = A_new.filter(x => !A_postid.includes(x));
...
// 2022cvt_022
		if (!(undefined !== H_log[this.m_log_index_planid]) || 0 == H_log[this.m_log_index_planid].length || 0 == "\\N".localeCompare(H_log[this.m_log_index_planid])) H_log[this.m_log_index_planid] = -1;
// 2022cvt_022
		if (!(undefined !== H_log[this.m_log_index_packetid]) || 0 == H_log[this.m_log_index_packetid].length || 0 == "\\N".localeCompare(H_log[this.m_log_index_packetid])) H_log[this.m_log_index_packetid] = -1;
