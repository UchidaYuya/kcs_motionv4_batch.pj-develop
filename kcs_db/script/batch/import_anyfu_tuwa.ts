...
(async () => {
const SCRIPT_NAMEJ = "エニーユーザーFUSION通話情報ファイルインポート";
const SCRIPTNAME = "import_anyfu_tuwa.php";

const ANYFU_DIR = "/anyfu/tuwa";
const ANYFU_PAT = "/^fcc_inv_.*_riyomr\\.(csv|txt)$/";
const FIN_DIR = "fin";
const LOG_DELIM = " ";
...
// 2022cvt_019
				...
				// if (preg_match(ANYFU_PAT, fileName)) {
...
// 2022cvt_015
	...
	if (EnableKousi == true) //kousiflg = 2:未登録 の電話はマスターから削除することになるので拾わない
		...
		//公私フラグの初期化
		{
			sql = "select telno, totelno, kousiflg from kousi_totel_master_tb where pactid=" + pactid + " and carid=" + ANY_CARRIER_ID + "and kousiflg != 2 order by telno, totelno";
...
//サービスごとの小計
{
	...
	// if (!("dataDirPact" in global)) dataDirPact = undefined;
// 2022cvt_015
	var buffer;
	try {
...
// 2022cvt_015
	...
	var path_2 = PATH.parse(path.base + ".txt");
	var fname_array = path_2.base.split("_");
...
// 2022cvt_015
	...
	//レコード数のチェック	// Modify * エニーユーザーはカラム数が違う
	//番号.サービス名
	//05. 06. 07. は明細レコード
...
// 2022cvt_015
		...
		if (record.length != 15 - 1) {
			logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + fileName + "のレコード数が異常(" + record.length + "!= 15-1).");
		...
		var hi_code, service_name = record[4 - 1].split("\\.", 2);
		// [hi_code, service_name] = split("\\.", record[4 - 1], 2);
...
// 2022cvt_015
				...
				if (doMeisaiRecord(record, pactid, A_commFileData, sum) == 1) {
					flock.unlockSync(fileName);
					// flock(fp, LOCK_UN);
					// fclose(fp);
					return 1;
...
function doMeisaiRecord(record: string[], pactid: string, A_commFileData: any[], sum: { [key: string]: number }) //コード表
//読込明細件数
//-- DEBUG -- * 表示してみる
//print "**** doMeisaiRecord **********************************\n";
//foreach( $record as $item ){ print "\"" .  $item . "\"\n"; }
//請求番号チェック
//請求番号が得られないので、このチェックは無し
//if( trim( $record[0] ) != $reqNumber ){
//$logh->putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ . LOG_DELIM . $pactid . LOG_DELIM . $pactname . LOG_DELIM . $billdate . LOG_DELIM
//. "請求番号が違います.(". $record[0] ."!=". $reqNumner .")." );
//return 1;
//}
//
//請求年月チェック
...
// 2022cvt_015
	var telno_view = record[5 - 1].trim();
// 2022cvt_015
	var telno = telno_view.replace(/-/g, "");
// 2022cvt_015
	var date = record[6 - 1].substring(0, 4) + "-" + record[6 - 1].substring(4, 2) + "-" + record[6 - 1].substring(6, 2) + " " + record[7 - 1].substring(0, 2) + ":" + record[7 - 1].substring(2, 2) + ":" + record[7 - 1].substring(4, 2);
// 2022cvt_015
	var totelno = record[9 - 1].trim();
// 2022cvt_015
	var toplace = record[8 - 1].trim();
// 2022cvt_015
	var tuwa_min = parseInt(record[11 - 1]);
// 2022cvt_015
	var tuwa_hour = +(tuwa_min / 60);
	tuwa_min -= tuwa_hour * 60;
	// 2022cvt_015
	var tuwa_sec = parseInt(record[12 - 1]);
	...
	if (tuwa_hour < 10) {
		...
		tuwa_hour_2 = tuwa_hour.toString();
	}

	if (tuwa_min < 10) {
		tuwa_min_2 = "0" + tuwa_min;
	} else {
		tuwa_min_2 = tuwa_hour.toString();
	}

	if (tuwa_sec < 10) {
		tuwa_sec_2 = "0" + tuwa_sec;
	} else {
		tuwa_sec_2 = tuwa_sec.toString();
	}

// 2022cvt_015
	var time = tuwa_hour_2 + tuwa_min_2 + tuwa_sec_2;
// 2022cvt_015
	var charge = parseInt(record[13 - 1]);

// 2022cvt_016
	if (undefined !== H_TEL_TYPE[record[14 - 1]]) //区分をtypeに入れる
		{
// 2022cvt_016
// 2022cvt_015
			var type = record[14 - 1];
		} else {
		logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + "未知の電話区分です.(" + record[14 - 1] + ").");
		return 1;
	}
...
// 2022cvt_016
	A_commFileData.push(pactid + "\t" + telno + "\t" + type + "\t" + date + "\t" + totelno + "\t" + toplace + "\t" + time + "\t" + charge + "\t" + ANY_CARRIER_ID + "\t" + kousiflg + "\n");
	sum.value += charge;
	ReadMeisaiCnt++;
	return 0;
};

function doTotalRecord(record: (string | number)[], hi_code: any, TotalUp: number) //-- DEBUG -- * 表示してみる
//print "==== doTotalRecord ================================\n";
//foreach( $record as $item ){ print "\"" .  $item . "\"\n"; }
//少数の累積誤差があるので、上下一円までを許容する
{
	// if (!("logh" in global)) logh = undefined;
	...
	// if (!("billdate" in global)) billdate = undefined;
// 2022cvt_015
	var total = +record[13 - 1];
// 2022cvt_015
	var total2 = +TotalUp;

	if (total + 1 < total2 || total - 1 > total2) {
		logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + "会社合計金額が異なります.(" + total + "!=" + total2 + ").");
		return 1;
	}
...
// 2022cvt_015
		...
		{
// 2022cvt_015
			var sql = "select kousiptn, COALESCE(kousiflg,'(null)') as nkflag from tel_tb where pactid=" + pactid + " and carid=" + ANY_CARRIER_ID + " and telno='" + telno + "'";
// 2022cvt_015
			var H_result = await dbh.getHash(sql, true);
...
// 2022cvt_015
			...
			if (undefined !== tel_kousiflg == false || tel_kousiflg == "(null)") //print "DEBUG_02: default_kousiflg=". $default_kousiflg .", and  pattern_id=". $pattern_id ."\n";
				...
				{
					sql = "select kousiflg, patternid from kousi_default_tb where pactid=" + pactid + " and carid=" + ANY_CARRIER_ID;
					H_result = await dbh.getHash(sql, true);
...
// 2022cvt_015
			...
			if (tel_kousiflg == 0) //kousi_pattern_tb からフラグを取得
				...
				{
					sql = "select comhistflg, comhistbaseflg from kousi_pattern_tb where patternid=" + pattern_id + " and carid=" + ANY_CARRIER_ID;
					H_result = await dbh.getHash(sql, true);
...
// 2022cvt_015
		...
		} else //print "DEBUG_06: comhistbaseflg base, kousiflg=". $kousiflg ."\n";
		{
			if (totelno != "") //「未登録」で kousi_totel_master に電話を登録する。
				...
				{
					A_totelFileData.push(pactid + "\t" + telno + "\t" + ANY_CARRIER_ID + "\t" + totelno + "\t" + "2" + "\n");
...
function doDelete(A_pactDone: any[], db: ScriptDB) //commhistory_X_tb から削除する
//delete失敗した場合
//未登録電話のみ削除する
//delete失敗した場合
{
	...
	// if (!("logh" in global)) logh = undefined;
// 2022cvt_015
	var sql_str = "delete from " + commhistory_tb + " where pactid in (" + A_pactDone.join(",") + ") and carid = " + ANY_CARRIER_ID;
// 2022cvt_015
...
// 2022cvt_016
	...
	sql_str = "delete from kousi_totel_master_tb where pactid in (" + A_pactDone.join(",") + ") and carid = " + ANY_CARRIER_ID + " and kousiflg = 2";
