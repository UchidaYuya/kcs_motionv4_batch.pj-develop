...
import { DATA_LOG_DIR, DATA_DIR, IMPORT_NTTCOM_WLD, DATA_EXP_DIR } from '../../db_define/define';
...
(() => {
	...
	{
		// if (ereg("^-e=", process.argv[cnt]) == true) //モード文字列チェック
		if (process.argv[cnt].match("^-e=")) //モード文字列チェック
		{
	
			// var mode = ereg_replace("^-e=", "", process.argv[cnt]).toLowerCase();
		...
		if (process.argv[cnt].match("^-a=") ) //バックアップの有無のチェック
			{
	
				var g_anbun = process.argv[cnt].replace("^-a=", "").toLowerCase();
	...
	var cmd = (PHP + " " + IMPORT_NTTCOM_WLD + co_arg);
	...
	A_pactDone = A_pactDone.filter(function (value, index, self) {
		return self.indexOf(value) === index;
	});
	// A_pactFailed = array_unique(A_pactFailed);
	A_pactFailed = A_pactFailed.filter(function (value, index, self) {
		return self.indexOf(value) === index;
	});
	...
	//重複を除く.
	{
		...
		//一気に配列に読み込む.
		{
			...
			if (!fs.existsSync(tmp_commhistory)) //ファイルが存在しなければ処理をスキップ.
				{
					continue;
				}

			// A_tmpBuf = array_merge(A_tmpBuf, file(tmp_commhistory));
			A_tmpBuf = A_tmpBuf.concat(fs.readFileSync(tmp_commhistory).toString().split('\r\n'));
		}
		...
		for (var line of A_tmpBuf) {
			...
			fs.writeSync(fp_commhistory, line);
		}
	...
	function doBackup(db) //tel_details_X_tb をエクスポートする
	{
		...
		if (expDataByCursor(sql, outfile, db) != 0) //ロールバック
			//ロック解除
			{
	...
	async function doDelete(pactid, db: ScriptDB) //tel_Detail_X_tbから削除
	//delte失敗した場合
	//delte失敗した場合
	//delte失敗した場合
	{
		...
		if (db.isError()) //ロールバック
			{
				...
				logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + LOG_DELIM + billdate + LOG_DELIM + commhistory_tb + "のデリートに失敗しました、");
				db.rollback();
				return 1;
			}
		...
		if (db.isError()) //ロールバック
			{
				...
				logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + LOG_DELIM + billdate + LOG_DELIM + telX_tb + "のデリートに失敗しました、");
				db.rollback();
				return 1;
			}
	...
	async function doImport(file_telX, file_teldetails, file_commhistory, db: ScriptDB) //$tel_columns =
	...
	//commhistory_tbへのインポート
	{
		...
		var stat = fs.statSync(file_commhistory);
		if (stat.size > 0) {
			...
			if (await doCopyInsert(commhistory_tb, file_commhistory, commhisotry_col, db) != 0) //ロールバック
				//ロック解除
				//lock(false, $db);
				{
	...
	//インサート処理おしまい、実質的な処理はここで行われる.
	{
		...
		//インサート行の追加
		{
			...
			if (await ins.insert(H_ins) == false) {
				...
				logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid_in + LOG_DELIM + LOG_DELIM + billdate + LOG_DELIM + filename + "インサート中にエラー発生、データ=" + line);
				// fclose(fp);
				return 1;
			}
		}
	...
	function finalData(pactid, pactDir, finDir) //同名のファイルが無いか
	{
		...
		for (var fname of dirh) {
			...
			if (stat.isDirectory()) //ファイル名の先頭文字が適合するものだけ
				{
	...
	function usage(comment, db: ScriptDB) //print "Usage) " . $process["argv"][0] . " -e={O|A} -y=YYYYMM -p={all|PACTID} -b={Y|N} -t={N|O}\n";
	...
	//ロック解除
	{
		...
		// print("\n" + comment + "\n\n");
		// print("Usage) " + process.argv[0] + " -e={O|A} -y=YYYYMM -p={all|PACTID} -b={Y|N}\n");
