...
const SCRIPT_NAMEJ = "NTT-Comインポートメイン";
...
import importNTTComTuwaModel from './import_nttcom_tuwamodel';
import { importBaseModel } from './import_basemodel';

import { DATA_LOG_DIR, DATA_DIR, IMPORT_NTTCOM_BILL } from '../../db_define/define';
import { execSync } from 'child_process';

(() => {
	const LOG_DELIM = " ";
	const PHP = "php";
	const NTTCOM_FIX_CARID = 9;
	...
	for (var i = 1; i < process.argv.length; i++) //請求用の引数.
	//通話用の引数、-t を除く.
	{
		co_arg += " ";
	...
	var cmd = (PHP + " " + IMPORT_NTTCOM_BILL + co_arg);
	// system(cmd, status);
	...
	var O_base = new importBaseModel();
	O_base.setArgv(process.argv);
	O_base.setArgvCheckList(["e", "y", "p", "b"]);
	O_base.checkArgv(6);
	O_base.checkArgvDetail();
	O_base.setTableName();

	var H_argv = O_base.getArgv();

	var O_tuwa = new importNTTComTuwaModel(O_base);
	O_tuwa.setDataDir(O_tuwa.getDataDir());
	O_tuwa.getTargetPact(H_argv.targetpact);
	O_tuwa.setDataDir(DATA_DIR + "/" + H_argv.billdate + "/NTT_com");

	var H_pact = O_tuwa.getPact();
	O_tuwa.initialize();
	O_tuwa.setCarrierID(NTTCOM_FIX_CARID);
	O_tuwa.setBillDirStr("tuwa");
	O_tuwa.execMainProc(H_pact, O_tuwa.getEraceFlag());
