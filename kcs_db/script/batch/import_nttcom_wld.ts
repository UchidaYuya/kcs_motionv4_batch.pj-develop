...
const SCRIPT_NAMEJ = "NTT-Com国際電話インポートメイン";
...
import { DATA_LOG_DIR, IMPORT_NTTCOM_WLD_BILL } from '../../db_define/define';
...
(() => {
	...
	// var cmd = escapeshellcmd(PHP + " " + IMPORT_NTTCOM_WLD_BILL + co_arg);
	var cmd = (PHP + " " + IMPORT_NTTCOM_WLD_BILL + co_arg);
	// system(cmd, status);
	var status = 0;
	try {
		execSync(cmd);
...
// logh.putError(G_SCRIPT_END, SCRIPT_NAMEJ + LOG_DELIM + pactid_in + LOG_DELIM + LOG_DELIM + billdate + LOG_DELIM + "\u51E6\u7406\u5B8C\u4E86.");
	...
	function usage(comment, db: ScriptDB) //print "Usage) " . $_SERVER["argv"][0] . " -e={O|A} -y=YYYYMM -p={all|PACTID} -b={Y|N} -t={N|O}\n";
	//追加 20071003iga
