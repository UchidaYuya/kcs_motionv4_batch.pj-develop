...
import { G_AUTH_ASP } from "../batch/lib/script_common";
...
// 2022cvt_015
					...
					// backup = ereg_replace("^-b=", "", _SERVER.argv[cnt]).toLowerCase();
...
// 			clearstatcache();// 2022cvt_012
	...
	//処理する契約ＩＤを取得する
	{
		...
		var dirh = fs.readdirSync(HAND_DIR + "/" + pactid);
...
// 2022cvt_028
// 2022cvt_019
			if (fs.statSync(dataDirPact + "/" + fileName).isFile() && fileName.match("/^CardHandMeisai.*\\.txt/")) {
...
// 			clearstatcache();// 2022cvt_012
	...
	if (A_billFile.length == 0) {
		if (DEBUG_FLG) {
			logging("WARNING: 対象ファイルがみつかりません（");
		}
...
// 2022cvt_015
			...
			if (A_line[0] != A_pactid[cnt]) {
				if (DEBUG_FLG) {
					logging("ERROR: ファイルに記載されている契約ID(" + A_line[0] + ")とファイルが置かれたディレクトリ(" + A_pactid[cnt] + ")が異なります " + A_billFile[fcnt] + "(" + A_line[1] + "の行)");
				}
...
// 2022cvt_015
			var usedate = "\"" + A_line[4].substring(0, 4) + "-" + A_line[4].substring(4, 2) + "-" + A_line[4].substring(6, 2) + "\"";
...
// 2022cvt_019
			if (-1 !== CARD_result.indexOf(A_line[2]) == false && !card_xx_write_buf.match("/" + A_line[2] + "/")) //card_xx_tbへのコピー文のバッファ
				{
			...
			if (target == "n") //card_tbにデータが無いときはコピー文作成
				{
// 2022cvt_019
					if (-1 !== CARD_now_result.indexOf(A_line[2]) == false && !card_write_buf.match("/" + A_line[2] + "/")) {
						copy_buf = A_pactid[cnt] + "\t" + trg_post + "\t" + A_line[2] + "\t" + A_line[3] + "\t" + nowtime + "\t" + nowtime + "\tfalse\n";
		...
		if (rtn2 != 0) //ロールバック
			{
				...
				logh.putError(G_SCRIPT_ERROR, SCRIPT_FILENAME + " ETC手入力請求データ取込処理 " + cardusehistoryX_tb + "のデータインポートに失敗しました ");
				dbh.rollback();
				throw process.exit(1);// 2022cvt_009
			} else {
			if (DEBUG_FLG) {
...
// 2022cvt_028
// 2022cvt_019
				if (fs.statSync(dataDir + "/" + copyfileName).isFile() && copyfileName.match("/CardHandMeisai/")) {
...
function checkMeisaiFormat(A_line: string[]) //1,お客様番号
{
// 2022cvt_015
	var errcnt = 0;
...
// 2022cvt_019
	...
	// if (preg_match("/(\\D)/", A_line[1]) == true) {
		if (DEBUG_FLG) {
			logging("ETCカードの種類のフォーマットが違います " + A_line[1]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (!A_line[4].match("/^(\\d){8}$/")) {
	// if (preg_match("/^(\\d){8}$/", A_line[4]) == false) {
		if (DEBUG_FLG) {
			logging("利用日付のフォーマットが違います " + A_line[4]);
		}
		errcnt++;
	}

// 2022cvt_019
	if (A_line[5] != "" && !A_line[5].match("/^(\\d)(\\d):(\\d)(\\d)$/")) {
	// if (A_line[5] != "" && preg_match("/^(\\d)(\\d):(\\d)(\\d)$/", A_line[5]) == false) {
		if (DEBUG_FLG) {
			logging("利用時間のフォーマットが違います " + A_line[5]);
		}
		errcnt++;
	}
