...
import { G_AUTH_ASP, G_CODE_ASP, G_CODE_ASX, G_CODE_TAX, G_EXCISE_RATE } from "../batch/lib/script_common";
...
(async () => {
const SCRIPT_NAMEJ = "FUSION請求情報ファイルインポート";
const SCRIPTNAME = "import_fusion_bill.php";

const FUSION_DIR = "/fusion/bill";
...
// 2022cvt_015
						var seikyuNo = fname_array[1];

						if (undefined !== A_billFile[seikyuNo]) {
							fname_array = A_billFile[seikyuNo];
...
// 2022cvt_015
		...
		if (record.length != 12) {
			logh.putError(G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + fileName + "のレコード数が異常(" + record.length + "!= 12).");
		...
		var hi_code, service_name = record[5].split("\\.", 2);
		// [hi_code, service_name] = split("\\.", record[5], 2);
		var lo_code, utiwake_name = record[7].split("\\.", 2);
		// [lo_code, utiwake_name] = split("\\.", record[7], 2);
...
// 2022cvt_015
		...
		if (hi_code != 92 && hi_code > 90 && 100 > hi_code) {
...
function doTelRecord(record: Array<any>, code: string, pactid: string, A_fileData: Array<any>, sum: { [key: string]: number }, kazei_sum: { [key: string]: number }) //請求先番号
//読込電話件数
//読込明細件数
//親番号チェックフラグ
//20110414
...
function doSumRecord(record: Array<any>, pactid: string, A_fileData: Array<any>, sum: { [key: string]: number }, kazei_sum: { [key: string]: number }) //-- DEBUG -- * 表示してみる
//print "**** doSumRecord **********************************\n";
//foreach( $record as $item ){ print "\"" .  $item . "\"\n"; }
//電話番号
//ハイフン付きの電話番号を保持
...
function doTotalRecord(record: Array<any>, hi_code: string, TotalUp: number) //請求書に記載されている消費税合計
//global $logh;
//global $pactid;
//global $pactname;
//global $billdate;
...
function inTelData(A_array: Array<any>, telno: string) //存在しなかった
{
// 2022cvt_015
...
// 2022cvt_015
	...
	async function doCalc(pactid: string, A_fileData: Array<any>) //読み込んだ電話を追加してゆく
//ASP利用料金
//データが空のときは終了.
//電話番号（１回前の）
//電話毎の消費税
...
// 2022cvt_015
	...
	//var keys = Object.keys(A_fileData);
	A_fileData.sort( cmpfileData );
	// if (!keys.sort( cmpfileData ) {
	// // if (!A_fileData.sort("cmpfileData")) {
	// 	logh.putError(ScriptLog.G_SCRIPT_WARNING, SCRIPT_NAMEJ + LOG_DELIM + pactid + LOG_DELIM + pactname + LOG_DELIM + billdate + LOG_DELIM + "明細データの電話番号ソートに失敗しました.");
	// 	return 1;
	// }
...
function writeInsFile(pactid: string, A_calcData: Array<any>) //20110414
//データが空のときは終了.
//現在の日付を得る
//各データを書き出す
{
...
function doDelete(A_pactDone: Array<any>, db: ScriptDB) //delte失敗した場合
{
	// if (!("teldetailX_tb" in global)) teldetailX_tb = undefined;
	// if (!("logh" in global)) logh = undefined;
// 2022cvt_015
