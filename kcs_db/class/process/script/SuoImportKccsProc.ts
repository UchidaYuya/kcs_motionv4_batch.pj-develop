...
import {SuoImportKccsView} from '../../view/script/SuoImportKccsView';
import {SuoImportKccsModel} from '../../model/script/SuoImportKccsModel';

export class SuoImportKccsProc extends ProcessBaseBatch {

	O_View: SuoImportKccsView;
	O_Model: SuoImportKccsModel;
	...
	//Modelの生成
	{
		...
		this.O_View = new SuoImportKccsView();
		this.O_Model = new SuoImportKccsModel(this.get_MtScriptAmbient());
	}
	...
	//終了
	{
		...
		var dataDir = this.getSetting().get("KCS_DIR") + this.getSetting().get("KCS_DATA") + "/" + this.BillDate + this.getSetting().get("KCCS_DIR") + "/";
		...
		if (this.isDirCheck(dataDir) == false) {
			...
			this.errorOut(1000, this.getSetting().get("KCCS") + "請求データファイルディレクトリ" + dataDir + "がみつかりません\n", 0, "", "");
		...
		var H_utiwake = O_BillModel.getUtiwake(this.getSetting().get("A_CARID_LIST"));
		...
		//合計金額チェック
		{
			...
			var H_telnoX = O_TelModel.getCaridTelno(A_pactid[pactCounter], tableNo, this.getSetting().get("A_CARID_LIST"));
			...
			if (-1 !== Object.keys(H_pactFunc).indexOf("fnc_asp") == true) //ＡＳＰ料金を取得
				{
					aspFlg = true;
					aspCharge = O_BillModel.getAspCharge(A_pactid[pactCounter], this.getSetting().get("A_CARID_LIST"));
			...
			if ("N" == this.TargetTable) //現在テーブルより電話番号マスターを取得する array(CARID => array(telno))
				...
				//電話管理に登録済み電話番号の部署ＩＤを取得する（キャリアが異なる同一電話番号が異なる部署ＩＤのものは除外）
				{
					...
					H_telno = O_TelModel.getCaridTelno(A_pactid[pactCounter], undefined, this.getSetting().get("A_CARID_LIST"));
					
					// var A_outPutTel = Array();
					A_outPutTel = Array();
					// var H_postidUniq = O_TelModel.getTelnoPostid(A_pactid[pactCounter], undefined);
					H_postidUniq = O_TelModel.getTelnoPostid(A_pactid[pactCounter], undefined);
				}
			...
			//tel_X_tb へ登録する必要有りの場合
			{
				...
				//再掲区分が通常の場合は金額チェック用に料金を集計する
				{
					...
					A_outPutTelDetails.push({
						...
						taxkubun: this.getSetting().get("A_TAX_KUBUN")![H_utiwake[carid][H_editAllFileData[telno][detailno].code].taxtype],
						detailno: detailno,
						recdate: now,
						carid: carid,
						tdcomment: undefined,
					...
					if (this.getSetting().get("SAIKEI_NORMAL") == H_utiwake[carid][H_editAllFileData[telno][detailno].code].codetype) {
				...
				if (true == telAddFlgX) //tel_X_tb 出力用配列へ格納
					//フラグを戻す
					{
						if (this.getSetting().get("KCCS_CARID") == carid) {
							var arid = this.getSetting().get("KCCS_ARID");
							var cirid = this.getSetting().get("KCCS_CIRID");
						} else if (this.getSetting().get("KCCS_NTTEAST_CARID") == carid) {
							arid = this.getSetting().get("KCCS_NTTEAST_ARID");
							cirid = this.getSetting().get("KCCS_NTTEAST_CIRID");
						} else if (this.getSetting().get("KCCS_NTTWEST_CARID") == carid) {
							arid = this.getSetting().get("KCCS_NTTWEST_ARID");
							cirid = this.getSetting().get("KCCS_NTTWEST_CIRID");
						}
				...
				if ("N" == this.TargetTable && true == telAddFlg) //tel_tb 出力用配列へ格納
					...
					{
						if (this.getSetting().get("KCCS_CARID") == carid) {
							arid = this.getSetting().get("KCCS_ARID");
							cirid = this.getSetting().get("KCCS_CIRID");
						} else if (this.getSetting().get("KCCS_NTTEAST_CARID") == carid) {
							arid = this.getSetting().get("KCCS_NTTEAST_ARID");
							cirid = this.getSetting().get("KCCS_NTTEAST_CIRID");
						} else if (this.getSetting().get("KCCS_NTTWEST_CARID") == carid) {
				...
				if (true == aspFlg) //合計用に２つ進める
					//ASP利用料を出力
					{
						...
						A_outPutTelDetails.push({
							...
							codename: H_utiwake[carid][this.getSetting().get("UTIWAKE_ASP_CODE")].name,
							charge: aspCharge,
							// taxkubun: this.getSetting().A_TAX_KUBUN[H_utiwake[carid][this.getSetting().UTIWAKE_ASP_CODE].taxtype],
							taxkubun: this.getSetting().get("A_TAX_KUBUN")![H_utiwake[carid][this.getSetting().get("UTIWAKE_ASP_CODE")].taxtype],
							detailno: detailno,
							recdate: now,
							carid: carid,
							tdcomment: undefined,
						...
						if (0 != asxCharge) //ASP利用料消費税を出力
							{
								...
								A_outPutTelDetails.push({
									...
									codename: H_utiwake[carid][this.getSetting().get("UTIWAKE_ASX_CODE")].name,
									charge: asxCharge,
									// taxkubun: this.getSetting().A_TAX_KUBUN[H_utiwake[carid][this.getSetting().UTIWAKE_ASX_CODE].taxtype],
									taxkubun: this.getSetting().get("A_TAX_KUBUN")![H_utiwake[carid][this.getSetting().get("UTIWAKE_ASX_CODE")].taxtype],
									detailno: detailno,
									recdate: now,
									carid: carid,
									tdcomment: undefined,
		...
		if ("O" == this.Mode) {
			O_BillModel.delTelDetailsData(A_pactDone, tableNo, this.getSetting().get("A_CARID_LIST"));
		}
