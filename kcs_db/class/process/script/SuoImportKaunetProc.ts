...
import {SuoImportKaunetView} from '../../view/script/SuoImportKaunetView';
import {SuoImportKaunetModel} from '../../model/script/SuoImportKaunetModel';

export class SuoImportKaunetProc extends ProcessBaseBatch {

	O_View: SuoImportKaunetView;
	O_Model: SuoImportKaunetModel;
	...
	{
		super(H_param);
		this.getSetting().loadConfig("kaunet");
		this.O_View = new SuoImportKaunetView();
		this.O_Model = new SuoImportKaunetModel(this.get_MtScriptAmbient());
	}
	...
	//終了
	{
		// this.infoOut("\uFF2B\uFF41\uFF55\uFF4E\uFF45\uFF54\u8CFC\u8CB7\u30C7\u30FC\u30BF\u30A4\u30F3\u30DD\u30FC\u30C8\u958B\u59CB\n", 1);
		this.infoOut("Ｋａｕｎｅｔ購買データインポート開始\n", 1);
		...
		var dataDir = this.getSetting().get("KCS_DIR") + this.getSetting().get("KCS_DATA") + "/" + this.BillDate + this.getSetting().get("KOUBAI_KAUNET_DIR") + "/";

		// if (this.isDirCheck(dataDir, "rw") == false) {
		if (this.isDirCheck(dataDir) == false) {
			// this.errorOut(1000, this.getSetting().KOUBAI_KAUNET + "\u8ACB\u6C42\u30C7\u30FC\u30BF\u30D5\u30A1\u30A4\u30EB\u30C7\u30A3\u30EC\u30AF\u30C8\u30EA\uFF08" + dataDir + "\uFF09\u304C\u307F\u3064\u304B\u308A\u307E\u305B\u3093\n", 0, "", "");
			this.errorOut(1000, this.getSetting().get("KOUBAI_KAUNET") + "請求データファイルディレクトリ" + dataDir + "がみつかりません\n", 0, "", "");
		...
		var A_outPutPurchaseDetails;
		var A_outPutPurchaseX;
		var O_PurchaseModel
...
// 2022cvt_020
			...
			O_PurchaseModel = new PurchaseModel();
			var A_purchidX = O_PurchaseModel.getPurchid(A_pactid[pactCounter], this.getSetting().get("CARID"), "false", tableNo);
			var A_purchidXDel = O_PurchaseModel.getPurchid(A_pactid[pactCounter], this.getSetting().get("CARID"), "true", tableNo);
			var aspFlg = false;
			...
			A_outPutPurchaseDetails = Array();
			// var A_outPutPurchaseX = Array();
			A_outPutPurchaseX = Array();
			// var A_undoPurchidX = Array();
			...
			if ("N" == this.TargetTable) //現在テーブルより削除されていない購買ＩＤマスターを取得する array(purchid)
				...
				//削除フラグをfalseにする必要のある購買ＩＤリスト
				{
					// var A_purchid = O_PurchaseModel.getPurchid(A_pactid[pactCounter], this.getSetting().get("CARID"), "false");
					A_purchid = O_PurchaseModel.getPurchid(A_pactid[pactCounter], this.getSetting().get("CARID"), "false");

					// var A_purchidDel = O_PurchaseModel.getPurchid(A_pactid[pactCounter], this.getSetting().get("CARID"), "true");
			...
			// for (var koubai_id of Object.values(A_koubai_id)) //購買ＩＤを文字列認識させる
			...
			//ASP料金明細が発生する場合
			{
				...
				//売上ＮＯ一覧を１件ずつ処理 END
				{
					...
					// for (var uriageno of Object.values(A_uriageno)) //消費税対象額は売上ＮＯ毎に初期化
					for (var uriageno of A_uriageno) //消費税対象額は売上ＮＯ毎に初期化
					//$sumCharge = $sumChargeEco = 0;
					//赤黒区分一覧を取得
					//赤黒区分一覧を１件ずつ処理
					//赤黒区分一覧を１件ずつ処理 END
					//売上ＮＯ毎に消費税を出力
					{
						...
						//売上行ＮＯ一覧を１件ずつ処理 END
						{
							...
							// for (var lineno of Object.values(A_lineno)) //明細毎の小計計算
							...
							{
								var charge = H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].charge * H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].itemsum;

								if (this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green1 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green2 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green3 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green4) //通常商品の場合
									{
								...
								if (this.getSetting().get("TAX_SOTOZEI") == H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].taxkubun) //消費税を加算
									{
										sumCharge = sumCharge + charge;
									}

								if (this.getSetting().get("AKADEN_ORDER") == akakuro) {
									// var comment = "\u8D64\u9ED2\u5143\u767A\u6CE8\u756A\u53F7\uFF1A" + H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].akakuroorder;
									var comment = "赤黒元発注番号：" + H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].akakuroorder;
								} else {
									comment = "";
								}
								...
								A_outPutPurchaseDetails.push({
									...
									green4: H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].green4
								});
								detailno++;
								outPurchdetailsCnt++;
								...
								if (true == purchaseAddFlgX) //purchase_X_tb 出力用配列へ格納
									//フラグを戻す
									{
										A_outPutPurchaseX.push({
											...
											username: H_editAllFileData[koubai_id][uriagebi][uriageno][akakuro][lineno].username,
						...
						if (0 != +(sumCharge * this.getSetting().get("excise_tax"))) //purchase_details_X_tb 出力用配列へ格納
							{
								...
								A_outPutPurchaseDetails.push({
									...
									code: code,
									codename: H_utiwake[this.getSetting().get("CARID")][code].name,
									charge: +(sumCharge * this.getSetting().get("excise_tax")),
									slipno: uriageno,
									itemcode: undefined,
									// itemname: "\u6D88\u8CBB\u7A0E",
									itemname: "消費税",
		...
		this.unLockProcess(this.O_View.get_ScriptName());
		this.set_ScriptEnd();
		// this.infoOut("\uFF2B\uFF41\uFF55\uFF4E\uFF45\uFF54\u8CFC\u8CB7\u30C7\u30FC\u30BF\u30A4\u30F3\u30DD\u30FC\u30C8\u7D42\u4E86\n", 1);
		this.infoOut("Ｋａｕｎｅｔ購買データインポート終了\n", 1);
