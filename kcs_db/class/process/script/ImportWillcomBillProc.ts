...
import ImportWillcomView from "../../view/script/ImportWillcomView";
import ImportWillcomModel from "../../model/script/ImportWillcomModel";
...
//@return void
//
export default class ImportWillcomBillProc extends ProcessBaseBatch {
	A_pactDone: any;
	exit: any;
	O_View: ImportWillcomView;
	O_Model: any
	A_pactid: any;
	H_pactid: any;
	BackUpFlg: string = "";
	...
	static SCRIPTNAME = "ImportWillcomBill";
	...
	//Modelの生成
	{
		...
		this.O_View = new ImportWillcomView();
		this.O_Model = new ImportWillcomModel(this.get_MtScriptAmbient());
	}
...
// 2022cvt_015
			for (var pcnt = 0 + 1; pcnt < this.A_pactid.length; pcnt++) //pactid が会社マスターに登録されていない場合（スクリプト続行 次のpactidへスキップ）
			{
...
// 2022cvt_015
					...
					//移動先ディレクトリ
					{
						...
						if ("O" == this.Mode) {
							this.getBillModel().delTelDetailsData([pid], this.tableNo, [this.getSetting().WILLCOM_CARID]);
...
// 2022cvt_015
		for (var fcnt = 0 + 1; fcnt < A_billFile.length; fcnt++) //ファイルデータ配列
		//未登録内訳コード取得
		{
...
// 2022cvt_016
				if (H_row.codetype == ImportWillcomModel.UNREGIST_CODETYPE) {
		...
		if (0 != this.A_unregist.length) {
			this.warningOut(1000, ImportWillcomBillProc.SCRIPTNAME + "::仮登録の内訳コードがあります\n", 1);
			return false;
		}
		...
		if (-1 !== Object.keys(this.getFuncModel().getPactFunc(pactid, undefined, false)).indexOf("fnc_asp") == true) //ASP料金を取得
			{
				aspFlg = true;
// 2022cvt_015
				var aspCharge = this.getBillModel().getAspCharge(pactid, this.getSetting().WILLCOM_CARID);
...
// 2022cvt_015
		...
		H_telnoX = this.getTelModel().getCaridTelno(pactid, this.tableNo, [this.getSetting().WILLCOM_CARID]);
		...
		if ("N" == this.TargetTable) //現在テーブルより電話番号マスターを取得する array(telno)
			...
			{
// 2022cvt_015
				var H_telno = this.getTelModel().getCaridTelno(pactid, undefined, [this.getSetting().WILLCOM_CARID]);
...
// 2022cvt_015
		for (var H_row of (A_allFileData)) //tel_X_tb に電話番号がない場合
		{
			...
			if (!!H_row.telno && -1 !== H_telnoX[this.getSetting().get("WILLCOM_CARID")].indexOf(H_row.telno) == false) //tel_X_tb へ登録する必要有り
				//電話番号リストに追加
				//現在テーブルにも追加する場合（過去テーブルに無いときだけ追加）
				{
					this.O_Model.setOutPut(this.manageX_tb, A_outPut[this.manageX_tb], H_row);
					H_telnoX[this.getSetting().get("WILLCOM_CARID")].push(H_row.telno);

					if ("N" == this.TargetTable) //tel_tb に電話番号がない場合
						{
							H_row.postid = rootPostid;

							if (-1 !== H_telno[this.getSetting().get("WILLCOM_CARID")].indexOf(H_row.telno) == false) //tel_tb へ登録する必要有り
								//電話番号リストに追加
								{
									this.O_Model.setOutPut(this.manage_tb, A_outPut[this.manage_tb], H_row);
									H_telno[this.getSetting().get("WILLCOM_CARID")].push(H_row.telno);
								}
						}
				}
		}
...
// 2022cvt_015
		for (var cnt = 0 + 1; cnt < A_allFileData.length; cnt++) //tel_amount_bill_tb へ登録
		{
// 2022cvt_015
			var H_row = A_allFileData[cnt];

			if (H_row.telno == "") {
				if (H_row.postname == ImportWillcomBillProc.TOTAL_POSTNAME && H_row.codename == ImportWillcomBillProc.TOTAL_CODENAME) {
			...
			} else //ASP使用料
				{
					...
					if (true == aspFlg && undefined !== A_allFileData[cnt + 1] && H_row.telno !== A_allFileData[cnt + 1].telno) {
						H_row.detailno += 2;
						H_row.code = ImportWillcomBillProc.ASPCODE;
						H_row.codename = ImportWillcomBillProc.ASPCODENAME;
						H_row.charge = aspCharge;
						this.O_Model.setOutPut(this.details_tb, A_outPut[this.details_tb], H_row);
						H_row.detailno++;
						H_row.code = ImportWillcomBillProc.ASXCODE;
						H_row.codename = ImportWillcomBillProc.ASXCODENAME;
	...
	async checkFileName(pactid: string, fileName: string) {
// 2022cvt_015
...
// 2022cvt_015
		// var A_file = preg_split("/_|\\./", H_path.basename);
		var A_file = H_path.base.split("/_|\\./");
...
// 2022cvt_015
		var A_clamps = await O_model.getClamps(pactid, this.getSetting().WILLCOM_CARID);
// 2022cvt_015
		var flag = false;
...
// 2022cvt_015
	...
	{
// 2022cvt_015
		var H_utiwake = this.getBillModel().getUtiwake([this.getSetting().get("WILLCOM_CARID")]);
		this.H_utiwake = H_utiwake[this.getSetting().get("WILLCOM_CARID")];
	}
