...
import {SuoImportAskulView} from '../../view/script/SuoImportAskulView';
import {SuoImportAskulModel} from '../../model/script/SuoImportAskulModel';

export class SuoImportAskulProc extends ProcessBaseBatch {

	O_View: SuoImportAskulView;
	O_Model: SuoImportAskulModel;
	PactId: string = "";
	BillDate: string = "";
	BackUpFlg: string = "";
	Mode: string = "";
	...
	//終了
	{
		...
		var dataDir = this.getSetting().get("KCS_DIR") + this.getSetting().get("KCS_DATA") + "/" + this.BillDate + this.getSetting().get("KOUBAI_ASKUL_DIR") + "/";
		...
		if (this.isDirCheck(dataDir) == false) {
			...
			this.errorOut(1000, this.getSetting().get("KOUBAI_ASKUL") + "請求データファイルディレクトリ" + dataDir + "がみつかりません\n", 0, "", "");
		...
		//正常処理が完了した pactid のみリストに追加
		{
			...
			//ASP料金明細が発生する場合
			{
				...
				if (-1 !== A_purchidX.indexOf(this.getSetting().get("LOGINID_HEADER") + koubai_id) == false) //削除済みの購買ＩＤとして登録が有る場合
					//購買ＩＤマスターに追加
					{
						if (-1 !== A_purchidXDel.indexOf(this.getSetting().get("LOGINID_HEADER") + koubai_id) == true) //削除を取り消す購買ＩＤリストに追加
							{
								A_undoPurchidX.push(this.getSetting().get("LOGINID_HEADER") + koubai_id);
							} else //purchase_X_tb へ登録する必要有り
							{
								purchaseAddFlgX = true;
							}

						A_purchidX.push(this.getSetting().get("LOGINID_HEADER") + koubai_id);
					}
				...
				if ("N" == this.TargetTable) //purchase_tb に登録する必要があるかどうか false：無、true：有
					//purchase_tb に購買ＩＤがない場合
					{
						...
						if (-1 !== A_purchid.indexOf(this.getSetting().get("LOGINID_HEADER") + koubai_id) == false) //削除済みの購買ＩＤとして登録が有る場合
							//購買ＩＤマスターに追加
							{
								if (-1 !== A_purchidDel.indexOf(this.getSetting().get("LOGINID_HEADER") + koubai_id) == true) //削除を取り消す購買ＩＤリストに追加
									{
										A_undoPurchid.push(this.getSetting().get("LOGINID_HEADER") + koubai_id);
									} else //purchase_tb へ登録する必要有り
									{
										purchaseAddFlg = true;
									}

								A_purchid.push(this.getSetting().get("LOGINID_HEADER") + koubai_id);
							}
					}
				...
				//売上ＮＯ一覧を１件ずつ処理 END
				{
					...
					//売上ＮＯ毎に消費税を出力
					{
						...
						{
							if (this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][lineno].green1 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][lineno].green2 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][lineno].green3 || this.getSetting().get("ECO_FLG") == H_editAllFileData[koubai_id][uriagebi][uriageno][lineno].green4) //通常商品の場合
								{
							...
							A_outPutPurchaseDetails.push({
								...
								purchid: this.getSetting().get("LOGINID_HEADER") + koubai_id,
